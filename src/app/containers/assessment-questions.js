...
import { selectMe, selectFetchingRecord, selectAssessmentQuestions, selectAssessment, selectQuestionTypes, selectCompanyConfig, selectBenchmarkTests,
					selectCoursesBase, selectVoiceRecording, selectAccessRoles, selectStandards } from '../store.js';
...
class Container extends Component {
  componentDidMount() {
      const {getPageLangs, langCode, personId, setMyVisitedPage, getStandards, assessmentId, getAssessment, assessmentQuestionsInit, getQuestionTypes, getBenchmarkTests, assignmentId} = this.props;
      ...
      assessmentQuestionsInit(personId, personId, assessmentId, assignmentId);
			...
			this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Assessment Questions`});
  }

  render() {
    const {fetchingRecord} = this.props;
    return fetchingRecord && !fetchingRecord.assessment  ? <AssessmentQuestionsView {...this.props} /> : null;
  }
}
