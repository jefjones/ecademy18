...
export default class SplitterOptionsBar extends Component {
    constructor(props) {
        super(props);

        this.state = {
            isShowingExplain: false,
        };

        this.handleNextEdit = this.handleNextEdit.bind(this);
        this.handlePrevEdit = this.handlePrevEdit.bind(this);
        this.handleSplitExplainClose = this.handleSplitExplainClose.bind(this);
        this.handleSplitExplainOpen = this.handleSplitExplainOpen.bind(this);
    }

    handleNextEdit() {
        this.props.jumpToEdit('NEXT');
    }

    handlePrevEdit() {
        this.props.jumpToEdit('PREV');
    }

    handleSplitExplainClose = () => this.setState({isShowingExplain: false})
    handleSplitExplainOpen = () => this.setState({isShowingExplain: true})

    render() {
        const {className, toggleLeftSidePanelOpen} = this.props;
        const {isShowingExplain} = this.state;
        ...
        return (
            <div className={classes(className, styles.container)}>
                <div className={styles.row}>
                    ...
                    <div>
                        <a onClick={this.handlePrevEdit}>
                            <Icon pathName={`arrow_up`} className={styles.downArrow}/>
                        </a>
                    </div>
                    <div>
                        <a onClick={this.handleSplitExplainOpen}>
                            <Icon pathName={`info`} className={styles.image}/>
                        </a>
                    </div>
                </div>
                {isShowingExplain &&
                    <MessageModal handleClose={this.handleSplitExplainClose} heading={<L p={p} t={`How to split a section/chapter?`}/>}
                       explainJSX={[<L p={p} t={`1. Click on any text where you want the new section to begin. The text you chose will be included in that new section.`}/>,<br/>,<br/>,
                         ...
                         <L p={p} t={`5. If you are ready to accept, click on the 'Submit' button at the top of the section list.`}/>]} onClick={this.handleSplitExplainClose} />
                }
            </div>
        )
    }
