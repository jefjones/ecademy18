...
import styles from './BehaviorIncidentListView.css';
...
class BehaviorIncidentListView extends Component {
  constructor(props) {
	    super(props);
	    ...
	    this.state = {
					...
					partialNoteText: props.studentChosenSession || '', //This is to gather the text but the search version below is the actual variable to filter the text.
					partialNoteSearch: props.studentChosenSession || '',
					studentPersonId: '',
          staffMemberId: '',
					fromDate: '',
					toDate: ''
	...
	componentDidUpdate() {
			const {personId, getBehaviorIncidents, studentPersonId, students} = this.props;
			const {timerId} = this.state;
			if (!timerId) this.setState({ timerId: setInterval(() => getBehaviorIncidents(personId), 10000) });
			if ((!this.state.studentPersonId || this.state.studentPersonId === guidEmpty) && studentPersonId && studentPersonId !== guidEmpty && students && students.length > 0) {
					...
					this.setState({ student, studentPersonId, showSearchControls: true });
			}
	}
	...
	handleChange = ({target}) => {
			...
			setStudentChosenSession(target.value);
	}

	handlePartialNameText = ({target}) => {
			this.setState({ partialNoteText: target.value });
	}
	...
	setPartialNameText = () => {
			const {partialNoteText} = this.state;
			this.setState({ partialNoteSearch: partialNoteText });
      this.props.setStudentChosenSession();
	}
	...
	clearFilters = () => {
			this.setState({ behaviorIncidentTypeId: '', studentPersonId: '', staffMemberId: '', fromDate: '', toDate: '', partialNoteSearch: ''})
      this.props.setStudentChosenSession();
	}

	handleNoteOpen = (assignmentName, note) => this.setState({isShowingModal_note: true, note, assignmentName })
	handleNoteClose = () => this.setState({isShowingModal_note: false, note: '', assignmentName: '' })
  ...
  handleDescriptionOpen = (noteHeading, noteDisplay) => this.setState({isShowingModal_description: true, noteHeading, noteDisplay })
  handleDescriptionClose = () => this.setState({isShowingModal_description: false, noteHeading: '', noteDisplay: '' })

	handleRemoveItemOpen = (behaviorIncidentId) => this.setState({isShowingModal_remove: true, behaviorIncidentId })
  handleRemoveItemClose = () => this.setState({isShowingModal_remove: false })
  handleRemoveItem = () => {
      const {removeBehaviorIncident, personId} = this.props;
      const {behaviorIncidentId} = this.state;
      removeBehaviorIncident(personId, behaviorIncidentId);
      this.handleRemoveItemClose();
  }

  render() {
    const {personId, myFrequentPlaces, setMyFrequentPlace, behaviorIncidents, behaviorIncidentTypes, students, staffMembers, fetchingRecord} = this.props;
		const {behaviorIncidentTypeId, partialNoteText, partialNoteSearch, studentPersonId,staffMemberId, fromDate, toDate, showSearchControls, isShowingModal_remove,
						isShowingModal_note, note, isShowingModal_picture, fileUrl, isShowingModal_description, noteHeading, noteDisplay} = this.state;
    ...
    if (localIncidents && localIncidents.length > 0) {
		        ...
		        data = localIncidents && localIncidents.length > 0 && localIncidents.map(m => {
                    ...
                    {value: <Link to={`behaviorIncidentAdd/${m.behaviorIncidentId}`}>
			              ...
			              {value: <a onClick={() => this.handleRemoveItemOpen(m.behaviorIncidentId)} className={styles.remove}>
                              ...
                              ? <div onClick={() => this.handleDescriptionOpen(m.accusedStudents.reduce((acc, s) => acc && acc.length > 0 ? acc += ', ' + s.label : s.label, ''), m.note)} className={globalStyles.link}>{m.note.substring(0,35) + '...'}</div>
                              : m.note
                    },
    ...
    return (
        <div className={styles.container}>
                ...
                <div>
                    ...
                    <div>
										...
										<div className={styles.row}>
												...
												<div onClick={this.setPartialNameText} className={styles.checkmarkPosition}>
										...
										<div>
												<div className={classes(styles.littleLeft, styles.moreTop, styles.row)}>
														<div className={styles.dateRow}>
																<DateTimePicker id={`fromDate`} label={<L p={p} t={`From date`}/>} value={fromDate} maxDate={toDate} onChange={(event) => this.changeDate('fromDate', event)}/>
														</div>
														<div className={styles.dateRow}>
																<DateTimePicker id={`toDate`} value={toDate} label={<L p={p} t={`To date`}/>} minDate={fromDate ? fromDate : ''} onChange={(event) => this.changeDate('toDate', event)}/>
														</div>
												</div>
										</div>
										<hr/>
						...
						<OneFJefFooter />
						{isShowingModal_note &&
								<MessageModal handleClose={this.handleNoteClose} heading={<L p={p} t={`Note`}/>}
									 explain={note} onClick={this.handleNoteClose} />
						}
            ...
            {isShowingModal_description &&
                <MessageModal handleClose={this.handleDescriptionClose} heading={noteHeading}
                   explain={noteDisplay} onClick={this.handleDescriptionClose} />
            }
            {isShowingModal_remove &&
								<MessageModal handleClose={this.handleRemoveItemClose} heading={<L p={p} t={`Remove this behavior incident?`}/>}
