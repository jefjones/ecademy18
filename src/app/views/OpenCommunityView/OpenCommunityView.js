...
export default class OpenCommunityView extends Component {
    constructor(props) {
        super(props);

        this.state = {
            modifyOpenCommunityEntryId: 0,
            tabsData: {
        ...
        this.handleSetToModifyRecord = this.handleSetToModifyRecord.bind(this);
    }

    handleTabChange(tabId) {
        const {tabsData} = this.state;
        this.setState({tabsData: {...tabsData, chosenTab: tabId}})
    }

    handleSetToModifyRecord(openCommunityEntryId) {
        const {tabsData} = this.state;
        this.setState({
            tabsData: {...tabsData, chosenTab: "ToSubmit"},
            modifyOpenCommunityEntryId: openCommunityEntryId
        })
    }
    ...
    render() {
        const {personId, workOptions, workList, saveOpenCommunityEntry, modifyEntry, declineIdleOptions, genreOptions, languageOptions,
                ...
                openCommunityFilterOptions, } = this.props;
        const {modifyOpenCommunityEntryId} = this.state;
//personConfig,
                ...
                <Accordion noShowExpandAll={true}>
                    ...
                    <AccordionItem title={<L p={p} t={`To Submit`}/>} count={tabEntryCounts[2]} isOpenCommunity={true}>
                        <OpenCommunityToSubmit workOptions={workOptions} workList={workList} saveOpenCommunityEntry={saveOpenCommunityEntry}
                            ...
                            onSubmitTabChange={() => this.handleTabChange('Submitted')} modifyOpenCommunityEntryId={modifyOpenCommunityEntryId}
                    ...
                    <AccordionItem title={<L p={p} t={`Submitted`}/>} count={tabEntryCounts[3]} isOpenCommunity={true}>
                        <OpenCommunitySubmitted personId={personId} openCommunityFull={openCommunityFull} removeOpenCommunityEntry={removeOpenCommunityEntry}
                            setToModifyRecord={this.handleSetToModifyRecord} updateChapterComment={updateChapterComment}/>
                    </AccordionItem >
                </Accordion>
                <OneFJefFooter />
            </section>
