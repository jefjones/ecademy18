...
export const selectEditWorksEditorsOneCount = (state, editTypeCount) => {
    ...
    state && state.length > 0 && state.forEach(w => {
        ...
        for(var i = 1; i < headings.length; i++) { //Notice that we start with 1 and not 0 since we picked up that up above for the work record.
...
export const selectWorksLangsEditorsOneCount = (state, editTypeCount, languageList) => {
    ...
    for(var rowIndex = 0; rowIndex < data.length; rowIndex++) { //Work by rowIndex
        for(var colIndex = 1; colIndex < headings.length; colIndex++) { //language (languageId) by column Index (colIndex)
            let editCounts = 0;
...
export const selectWorkLangsEditorsOneCount = (state, workId, editTypeCount, languageList) => {
    ...
    for(var rowIndex = 0; rowIndex < data.length; rowIndex++) { //language by rowIndex
        for(var colIndex = 1; colIndex < headings.length; colIndex++) { //editor (personId) by column Index (colIndex)
            work && work.length > 0 && work.forEach(w => {  //eslint-disable-line
                w.reportChapters && w.reportChapters.length > 0 && w.reportChapters.forEach(c => {
...
export const selectWorksEditorsCountLangs = (state) => {
    ...
    for(var rowIndex = 0; rowIndex < data.length; rowIndex++) { //Editor by rowIndex
        for(var colIndex = 1; colIndex < headings.length; colIndex++) { //language (languageId) by column Index (colIndex)
            let languages = [];
            let work = state.filter(w => w.workId === data[rowIndex][0].headingId); //eslint-disable-line
            work && work.length > 0 && work.forEach(w => {  //eslint-disable-line
