...
function GraduationRequirements(props) {
  const [isShowingModal_note, setIsShowingModal_note] = useState(false)
  const [courseName, setCourseName] = useState('')
  const [notes, setNotes] = useState('')

  const {emptyMessage, requirements, setContentAreaFilter} = props
          ...
          return (
              <div className={styles.container}>
  														...
  														return (
  																<tr key={i}>
  																		<td className={classes(styles.row, styles.cell)}>
  																				...
  																				{m.notes &&
  																						<div className={styles.link} onClick={() => handleNoteOpen(m.courseName, m.notes)}>
  								...
  								{isShowingModal_note &&
  		                <MessageModal handleClose={handleNoteClose} heading={courseName}
  		                   explain={notes} onClick={handleNoteClose} />
  		            }
              </div>
          )
}
