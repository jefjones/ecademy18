...
class GradebookEntryView extends Component {
    ...
    toggleCheckboxOverwrite = (field, studentPersonId) => {
        ...
        if (field.toLowerCase() === 'incomplete') {
            ...
            option.incomplete = false;
            option.passed = false;
            setGradeOverwrite(personId, course.courseScheduledId, studentPersonId, personConfig.intervalId, 'withdrawn', !this.hasGradeOverwrite('withdrawn', studentPersonId));
        }
    ...
    // onStandardLevelIncrement = (studentPersonId, assignmentId, standardLevelSequence=1) => { //This defaul was changed from 0 to 1.  I hope it still works that way now that the sequences have been increased by one to 1-n insetad of 0-n
      ...
      const {isShowingModal, instructions, assignmentName, studentPersonId, isShowingModal_instructions, penspringWorkId, isShowingModal_response,
							...
							scrollTop, set_studentPersonId, set_assignmentId, chosenStudentRow, showIncompeteWithdrawnPassed, showGradeOverwrite} = this.state;
      ...
      if (!isLevelOnly) {
          ...
          //
            ...
            clickFunction: () => this.chooseRecord(m.id),
						value: <div onClick={() => this.sendToStudentAssignmentView(m.personId)} className={classes(styles.link, styles.row, styles.lineheight)}>
                ...
                clickFunction: () => this.chooseRecord(m.id),
                          ...
                          {this.hasGradeOverwrite('incomplete', m.personId)
                              ...
                              : this.hasGradeOverwrite('passed', m.personId)
                                  ...
                                  : this.hasGradeOverwrite('withdrawn', m.personId)
                              ...
                              <div className={styles.row}>
                                  ...
                                  <div className={styles.checkboxPosition}>
          														<Checkbox
          																...
          																checked={this.hasGradeOverwrite('withdrawn', m.personId)}
          																onClick={() => this.toggleCheckboxOverwrite('withdrawn', m.personId)}
                                          checkboxClass={styles.checkboxItself} />
          												</div>
                                  <div className={styles.checkboxPosition}>
          														<Checkbox
            ...
            <div className={classes(globalStyles.pageTitle, styles.moreMarginBottom)}>
              	{`Gradebook ${course.gradingType === 'STANDARDSRATING' ? '(Standards-based)' : course.gradingType === 'PASSFAIL' ? '(Pass / Fail)' : ''}`}
						...
						<div className={styles.row}>
								...
								<ReactToPrint trigger={() => <a href="#" className={classes(styles.moreRight, styles.link)}><L p={p} t={`Print`}/></a>} content={() => this.componentRef}/>
            ...
            {(accessRoles.admin || accessRoles.facilitator) &&
                <div className={classes(styles.moreTop, styles.row)}>
                    ...
                    <Checkbox
                        ...
                        onClick={() => this.toggleCheckbox('showIncompeteWithdrawnPassed')}
                        labelClass={styles.labelCheckbox}
                        className={styles.checkbox} />
                    <Checkbox
                        ...
                        onClick={() => this.toggleCheckbox('showGradeOverwrite')}
                        labelClass={styles.labelCheckbox}
                        className={styles.checkbox} />
                </div>
            }
                ...
                {courseScheduledId !== "0" &&
								...
								{gradebook && gradebook.assignments && gradebook.assignments.length > 0 && courseScheduledId && !(fetchingRecord && fetchingRecord.gradebook) &&
										<div className={styles.row}>
												...
												<ButtonWithIcon icon={'checkmark_circle'} label={<L p={p} t={`Finalize`}/>} onClick={this.handleFinalizeGradeOpen}/>
