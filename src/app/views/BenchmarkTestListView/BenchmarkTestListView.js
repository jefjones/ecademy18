...
import styles from './BenchmarkTestListView.css';
...
class BenchmarkTestListView extends Component {
  constructor(props) {
	    super(props);

	    this.state = {
					scores: [],
					isShowingModal_display: false,
					hideSearch: true,
					filters: {
					}
	    }
	...
	// componentDidUpdate() {
	// 		const {benchmarkTestId, benchmarkTest} = this.props;
	// 		const {isParamBenchmarkTest} = this.state;
	//
	// 		if (!isParamBenchmarkTest && benchmarkTest && benchmarkTest.benchmarkTestId) {
	// 				this.setState({ isParamBenchmarkTest: true, benchmarkTestId, benchmarkTest})
	// 		}
	// }

	reorderSequence = (benchmarkTestId, event) => {
	    const {reorderBenchmarkTests, personId} = this.props;
	    reorderBenchmarkTests(personId, benchmarkTestId, event.target.value);
  }
	...
	handleDisplayClose = () => this.setState({isShowingModal_display: false, modalDisplay: '', modalOpen: false})
	handleDescriptionOpen = (modalDisplay) => this.setState({isShowingModal_display: true, modalTitle: 'Description', modalDisplay, modalOpen: true })
	handleShowListOpen = (modalTitle, list) => {
			...
			this.setState({isShowingModal_display: true, modalTitle, modalDisplay, modalOpen: true })
	}

	chooseRecord = (benchmarkTest) => {
			let userGradeLevels = doSort(benchmarkTest.gradeLevels, { sortField: 'sequence', isAsc: true, isNumber: true });
			benchmarkTest.fromGradeLevelId = userGradeLevels && userGradeLevels.length > 0 && userGradeLevels[0].gradeLevelId;
			benchmarkTest.toGradeLevelId = userGradeLevels && userGradeLevels.length > 0 && userGradeLevels[userGradeLevels.length-1*1].gradeLevelId;
			this.setState({ benchmarkTest });
	}

	handleAddOrUpdateBenchmarkTestOpen = (benchmarkTest) =>
		this.setState({ isShowingModal_addOrUpdate: true, benchmarkTestId: benchmarkTest && benchmarkTest.benchmarkTestId, modalOpen: true });

  handleAddOrUpdateBenchmarkTestClose = () => this.setState({isShowingModal_addOrUpdate: false, benchmarkTest: '', modalOpen: false })
  handleAddOrUpdateBenchmarkTest = (benchmarkTest) => {
      const {addOrUpdateBenchmarkTest, personId} = this.props;
			addOrUpdateBenchmarkTest(personId, benchmarkTest);
			this.handleAddOrUpdateBenchmarkTestClose();
  }

	changeBenchmarkTest = ({target}) => {
      let benchmarkTest = Object.assign({}, this.state.benchmarkTest);
			benchmarkTest[target.name] = target.value;
      this.setState({ benchmarkTest });
  }

	render() {
    const {fetchingRecord, personId, course={}, benchmarkTests, accessRoles={}, personConfig, companyConfig, sequences, removeBenchmarkTest,
						facilitators, myFrequentPlaces, setMyFrequentPlace, sharedTeachersBenchmarkTest} = this.props;
		const {isShowingModal_removeCourseDoc, hideSearch, filters={}, isShowingModal_display, modalDisplay, modalTitle, benchmarkTest={},
						isShowingModal_addOrUpdate, modalOpen} = this.state;
    ...
    let data = testListFiltered && testListFiltered.length > 0 && testListFiltered.map((m, i) => {
				...
				return ([
						{value: <div className={styles.smallWidth}>
												<ButtonWithIcon icon={'checkmark_circle'} label={''} onClick={() => this.chooseRecord(m)} addClassName={styles.smallButton}/>
										</div>
						},
						{value: <div className={styles.lessTop}>
												<SelectSingleDropDown
														...
														noBlank={true}
														options={sequences}
														onChange={(event) => this.reorderSequence(m.benchmarkTestId, event)}/>
										</div>,
								clickFunction: () => this.chooseRecord(m),
								cellColor: m.benchmarkTestId === benchmarkTestId ? 'highlight' : ''
						},
						{value: <div onClick={() => this.chooseRecord(m)} className={classes(styles.link, styles.row)}
						...
						{value: m.description && m.description.length > 35 ? m.description.substring(0,35) + '...' : m.description,
								clickFunction: () => { this.chooseRecord(m); m.description && m.description.length > 35 && this.handleDescriptionOpen(m.description)},
								notShowLink: m.description && m.description.length < 35,
								cellColor: m.benchmarkTestId === benchmarkTestId ? 'highlight' : ''
						},
						{value: m.standards && m.standards.length ? m.standards.length : '0',
								clickFunction: () => { this.chooseRecord(m); m.standards && m.standards.length && this.handleShowListOpen(<L p={p} t={`Standards`}/>, m.standards);},
								notShowLink: !(m.standards && m.standards.length),
								cellColor: m.benchmarkTestId === benchmarkTestId ? 'highlight' : ''
						},
						{value: m.sharedTeachers && m.sharedTeachers.length ? m.sharedTeachers.length : '0',
								clickFunction: () => {this.chooseRecord(m); m.sharedTeachers && m.sharedTeachers.length && this.handleShowListOpen(<L p={p} t={`Shared with Teachers`}/>, m.sharedTeachers)},
								notShowLink: !(m.sharedTeachers && m.sharedTeachers.length),
								cellColor: m.benchmarkTestId === benchmarkTestId ? 'highlight' : ''
						},
						{value: ratingAverage,
								clickFunction: () => {this.chooseRecord(m); m.classesCompleted && m.classesCompleted.length && this.handleShowListOpen(<L p={p} t={`Ratings`}/>, m.ratings)},
								notShowLink: !(m.ratings && m.ratings.length),
								cellColor: m.benchmarkTestId === benchmarkTestId ? 'highlight' : ''
						},
						{value: m.classesCompleted && m.classesCompleted.length ? m.classesCompleted.length : '0',
								clickFunction: () => {this.chooseRecord(m); m.classesCompleted && m.classesCompleted.length && this.handleShowListOpen(<L p={p} t={`Classes Completed`}/>, m.classesCompleted)},
						...
						{value: m.studentsCompleted && m.studentsCompleted.length ? m.studentsCompleted.length : '0',
								clickFunction: () => {this.chooseRecord(m); m.studentsCompleted && m.studentsCompleted.length && this.handleShowListOpen(<L p={p} t={`Students Completed`}/>, m.studentsCompleted)},
						...
						{value: m.publicCopies && m.publicCopies.length ? m.publicCopies.length : '0',
								clickFunction: () => {this.chooseRecord(m); m.publicCopies && m.publicCopies.length && this.handleShowListOpen(<L p={p} t={`Public Copies`}/>, m.publicCopies)},
    ...
    return (
        <div className={styles.container}>
						...
						<BenchmarkTestListMenu personId={personId} facilitators={facilitators} benchmarkTest={benchmarkTest} ownerPersonId={benchmarkTest.entryPersonId}
								...
								addOrUpdateBenchmarkTestOpen={this.handleAddOrUpdateBenchmarkTestOpen} companyConfig={companyConfig}
								assessmentId={benchmarkTest.assessmentId} modalOpen={modalOpen} />
						<div onClick={this.handleAddOrUpdateBenchmarkTestOpen} className={classes(styles.row, styles.link, styles.moreLeft, styles.moreTop)}>
						...
						{testListFiltered && testListFiltered.length > 9 && !(fetchingRecord && fetchingRecord.benchmarkTests) &&
								<div onClick={this.handleAddOrUpdateBenchmarkTestOpen} className={classes(styles.row, styles.link, styles.moreLeft)}>
										<Icon pathName={'plus'} fillColor={'green'} className={styles.iconSmaller}/>
										<L p={p} t={`Add a benchmark test`}/>
								</div>
						}
                ...
                <MessageModal handleClose={this.handleRemoveCourseDoc} heading={<L p={p} t={`Remove this Benchmark Test?`}/>}
                ...
                <BenchmarkTestAddModal handleClose={this.handleAddOrUpdateBenchmarkTestClose} handleSubmit={this.handleAddOrUpdateBenchmarkTest}
                    personId={personId} companyConfig={companyConfig} benchmarkTest={benchmarkTest} accessRoles={accessRoles}
										personConfig={personConfig} sequences={sequences} changeBenchmarkTest={this.changeBenchmarkTest}/>
            }
      	</div>
    );
  }
