...
import { selectMe, selectStudentBaseCourseRequests, selectCompanyConfig, selectAccessRoles, selectStudents, selectCourseTypes,
					...
					selectCurrentEnrollmentPreReq, selectDepartments, selectSeatsStatistics, selectDoNotAddCourses } from '../store.js';
...
class Container extends Component {
	  componentDidMount() {
		    const {getPageLangs, langCode, personId, setMyVisitedPage, getStudentBaseCourseRequests, studentPersonId,getCurrentEnrollmentPreReq, getGradRequirements, getDepartments,
								getLearningPathways} = this.props;
				if (studentPersonId) {
				...
				this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Student Base Course Request`});
	  }

	  render() {
	    	return <StudentBaseCourseRequestView {...this.props} />;
	  }
}
