...
import styles from './RegistrationPendingView.css';
...
export default class RegistrationPendingView extends Component {
    constructor(props) {
		...
		// changeSearch = ({target}) => {
	  //     let search = this.state.essageSearch;
		// 		search = target.value.toLowerCase();
	  //     this.setState({ search });
	  // }

		handleAcceptOpen = (registrationTableId, studentPersonId) => this.setState({ isShowingModal_accept: true, registrationTableId, studentPersonId})
		handleAcceptClose = () => this.setState({ isShowingModal_accept: false })
		handleAccept = () => {
				const {setRegistrationAcceptedDenied, personId} = this.props;
				const {registrationTableId, studentPersonId} = this.state;
				this.handleAcceptClose();
				setRegistrationAcceptedDenied(personId, registrationTableId, studentPersonId, "Approved", '');
		}

		handleDenyOpen = (registrationTableId, studentPersonId) => this.setState({ isShowingModal_deny: true, registrationTableId, studentPersonId})
		handleDenyClose = () => this.setState({ isShowingModal_deny: false })
		handleDeny = (regReviewNote, title) => {
				const {setRegistrationAcceptedDenied, personId} = this.props;
				const {registrationTableId, studentPersonId} = this.state;
				this.handleDenyClose();
				setRegistrationAcceptedDenied(personId, registrationTableId, studentPersonId, "DENIED", regReviewNote);
		}

		toggleCheckbox = () => {
				const {personId, updatePersonConfig, personConfig, getRegistrationPending} = this.props;
				updatePersonConfig(personId, 'ShowOnlyUnpaidReg', !personConfig.showOnlyUnpaidReg)
		...
		handleRegistrationStatus = (event, registrationTableId) => {
				const {updateRegistrationStatus, personId} = this.props;
				updateRegistrationStatus(personId, registrationTableId, event.target.value);
		}

		handleCheckboxGroup = (registrationStatusFilter) => this.setState({ registrationStatusFilter });
		...
		dataListChange = (field, studentIdList) => this.setState({ studentIdList })
      ...
      clearFilters = () => {
          const {personId, updatePersonConfig, getRegistrationPending} = this.props;
  				this.setState({
              partialNameText: '',
              studentIdList: [],
              selectedGradeLevels: [],
          ...
          updatePersonConfig(personId, 'ShowOnlyUnpaidReg', false);
          getRegistrationPending(personId);
    }

    render() {
      const {fetchingRecord, students, companyConfig} = this.props;
      const {personId, setRegistrationPaidDate, personConfig, registrationPending, registrationStati, myFrequentPlaces, setMyFrequentPlace} = this.props;
			...
			let {isShowingModal_accept, isShowingModal_deny, orderBy, orderDirection, registrationStatusFilter, partialNameText, studentIdList,
						selectedGradeLevels} = this.state;

			let headings = [
					{label: <L p={p} t={`Paid`}/>, tightText: true, clickFunction: () => this.resort('paidDate'), biggerClickArea: true},
					{label: <L p={p} t={`Entry Date`}/>, tightText: true, clickFunction: () => this.resort('entryDate'), biggerClickArea: true},
					{label: <L p={p} t={`Primary Guardian`}/>, tightText: true, clickFunction: () => this.resort('primaryGuardianLastFirstName'), biggerClickArea: true},
										...
										<div key={i}>
												...
												<div>
														<SelectSingleDropDown
																...
																height={`medium`}
																onChange={(event) => this.handleRegistrationStatus(event, m.registrationTableId)}/>
													</div>
										</div>
						))},
												...
												<div className={styles.row}>
														{f.regApprovedOrDenied
																...
																:	<div className={styles.row}>
																			...
																			<div onClick={() => this.handleAcceptOpen(m.registrationTableId, f.studentPersonId)}
																			...
																			<div onClick={() => this.handleDenyOpen(m.registrationTableId, f.studentPersonId)}
        ...
        return (
            <div className={styles.container}>
										...
										<div className={styles.rowWrap}>
												...
												<div className={classes(styles.moreLeft, styles.border, styles.moreTop)}>
														<CheckboxGroup
																...
																options={registrationStati || []}
																horizontal={true}
																onSelectedChanged={this.handleCheckboxGroup}
                            ...
                            <div className={classes(styles.row, styles.moreLeft)}>
                                <div onClick={this.clearFilters} className={classes(globalStyles.link, styles.moreTop)}>
		                ...
		                <MessageModal handleClose={this.handleAcceptClose} heading={<L p={p} t={`Accept this student's registration?`}/>}
		                   explainJSX={<L p={p} t={`Are you sure you want to accept this student's registration?`}/>} isConfirmType={true} onClick={this.handleAccept} />
		            }
								{isShowingModal_deny &&
										<TextareaModal handleClose={this.handleDenyClose} heading={<L p={p} t={<L p={p} t={`Deny this student's registration`}/>}/>}
												explainJSX={<L p={p} t={<L p={p} t={`Please provide an explanation for denying this registration.`}/>}/>} onClick={this.handleDeny}/>
								}
            </div>
        )
    };
