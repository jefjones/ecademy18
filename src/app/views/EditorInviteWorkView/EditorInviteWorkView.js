...
export default class EditorInviteWorkView extends Component {
  constructor(props) {
    super(props);
...
handleSend = () => {
    const {editorInviteWorkAssign} = this.props;
...
submitSend = () => {
    const {sendEditorInvite, owner_personId, editorInviteName, editorInviteWorkAssign} = this.props;
    sendEditorInvite(owner_personId, editorInviteName, editorInviteWorkAssign, () => browserHistory.push("/giveAccessToEditors"));
    this.setState ({
        isSending: true
    });
}

handleConfirmClose = () => this.setState({isShowingModal: false})
handleConfirmOpen = () => this.setState({isShowingModal: true})
...
render() {
    const {personId, owner_personId, languageId, updateFilterByField, clearFilters, setWorkCurrentSelected,
            ...
            personConfig} = this.props;

    const {isSending, isShowingModal} = this.state;
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.contact}>
                ...
                <div>
                    {!isSending && <ButtonWithIcon label={<L p={p} t={`Send`}/>} icon={'checkmark_circle'} className={styles.submitButton} onClick={this.handleSend}/>}
            ...
            {isShowingModal &&
                <MessageModal handleClose={this.handleConfirmClose} heading={<L p={p} t={`Send Invitation without Giving Access?`}/>}
                    ...
                    isConfirmType={true} onClick={this.submitSend}/>
            }
            <OneFJefFooter />
        </div>
    )
