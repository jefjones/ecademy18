...
class SelectPanel extends Component {
    ...
    selectAll = () => {
        ...
        const allValues = options && options.length > 0 && options.map(o => o.value);
    ...
    filteredOptions() {
        ...
        return options && options.length > 0 && options.filter(m => m.label.toLowerCase().indexOf(searchText.toLowerCase()) > -1);
    }
    ...
    componentWillUnmount() {
        this.props.getJustCollapsed ? this.props.getJustCollapsed() : () => {}; //Updated here in order for the editorAssign record to be saved if the user clicks outside of the drop down area anywhere else.
    }

    render() {
        const {focusIndex, searchHasFocus} = this.state;
        const {ItemRenderer, selectAllLabel, hideSelectAll} = this.props;
