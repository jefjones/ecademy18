...
import * as actionDiscussionEntries from '../actions/discussion-entries.js';
...
import { selectMe, selectFetchingRecord, selectDiscussionEntries, selectCompanyConfig, selectCoursesBase, selectVoiceRecording,
					selectAccessRoles, selectStudents, selectPersonConfig } from '../store.js';
...
class Container extends Component {
  componentDidMount() {
      const {getPageLangs, langCode, setMyVisitedPage, getVoiceRecording, courseEntryId, discussionEntriesInit, personId} = this.props;
      ...
      getVoiceRecording(personId, '');
			this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Discussion Entry`});
  }

  render() {
    //const {course, fetchingRecord, voiceRecording} = this.props;
    //voiceRecording &&
    // return course && course.courseName && fetchingRecord && fetchingRecord.discussionEntries === "ready"
		// 		? <DiscussionEntryView {...this.props} />
		// 		: null;
		return <DiscussionEntryView {...this.props} />;
  }
}
