...
import * as actionCourseAttendance from '../actions/course-attendance.js';
...
import { selectMe, selectStudents, selectCoursesScheduled, selectCompanyConfig, selectCourseAttendanceClassReport, selectPersonConfig,
					selectAccessRoles, selectFetchingRecord, selectStudentChosenSession} from '../store.js';
...
class Container extends Component {
    componentDidMount() {
        const {getPageLangs, langCode, personId, setMyVisitedPage, 	getAttendanceClassReport, courseScheduledId, getCoursesScheduled} = this.props;
        getPageLangs(personId, langCode, 'AttendanceClassReportView');
				...
				this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Course Attendance Class Report`});
    }

    render() {
        return <AttendanceClassReportView {...this.props} />;
    }
}
