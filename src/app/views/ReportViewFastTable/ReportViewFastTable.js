...
import styles from './ReportViewFastTable.css';
...
export default class ReportView extends Component {
  constructor (props, context) {
    ...
    this.recalcRowHeight = this.recalcRowHeight.bind(this);
  ...
  includeLineBreak ({
      ...
      if (cellData == null) {
        return ''
          ...
          let divided = cellData;
          if( Object.prototype.toString.call( divided ) !== '[object Array]' ) {
              divided = cellData.split("<br />");
          }
          return (divided.map((d, i) => <span key={i}>{i > 0 && <br />}{d}</span>))
      }
    }
...
includeMemberCard ({
  ...
  recalcRowHeight ({
    ...
    let lineHeight = this.props.data[index].lineCount > 2 ? (this.props.data[index].lineCount * 18) + 50 : 50;
    return lineHeight;
  }
    ...
    return (
        <div className={styles.outerContainer}>
            ...
            {tabs && <TabPage tabs={tabs} chosenTab={activeTab} onClick={setActiveTab}/>}
              <Table
                noRowsRenderer={this.noRecordsFound}
                sort={this._sort}
                sortBy={sortBy}
                ...
                height={450}
                headerHeight={20}
                rowHeight={activeTab === "households" ? this.recalcRowHeight : 50}
