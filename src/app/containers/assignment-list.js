...
import { selectMe, selectFetchingRecord, selectAssignments, selectCompanyConfig, selectCoursesBase, selectVoiceRecording, selectLanguageList,
					...
					selectIntervals, selectPersonConfig } from '../store.js';
...
const mapStateToProps = (state, props) => {
    ...
    let sequences = [];
    for(var i = firstSequence; i <= String(Number(sequenceCount) + Number(firstSequence) + 2*1); i++) {
...
class Container extends Component {
  ...
  componentDidMount() {
      const {getPageLangs, langCode, personId, getCourseWeightedScore, courseDocumentsInit, courseEntryId, assignmentsInit, getCoursesBase} = this.props;
	...
	componentDidUpdate() {
			const {personId, setMyVisitedPage, assignmentsInit, courseEntryId, course} = this.props;
			const {isInit} = this.state;

			if (course && course.courseName && (!isInit || courseEntryId !== this.state.courseEntryId)) {
					assignmentsInit(personId, courseEntryId);
					this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Assignments: ${course && course.courseName}`});
					this.setState({ isInit: true, courseEntryId });
			}
	}

  render() {
    return <AssignmentListView {...this.props} />;
	}
}
