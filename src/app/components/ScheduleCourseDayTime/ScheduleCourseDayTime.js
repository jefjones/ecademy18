...
export default class ScheduleCourseDayTime extends Component {
   constructor(props) {
	      super(props);
		...
		componentDidUpdate() {
				const {daysScheduled} = this.props;
				const {isInitialized} = this.state;
				if (!isInitialized && daysScheduled && daysScheduled.length > 0 && !!daysScheduled[0].dayOfTheWeek) {
						this.setState({ isInitialized: true, daysScheduled });
				}
		}

		setReoccurringByClassPeriod = (dayOfTheWeek, classPeriodId) => {
				const {classPeriods} = this.props;
				let daysScheduled = Object.assign([], this.state.daysScheduled);
				...
				if (classPeriod && classPeriod.startTime) {
						const {setDaySchedule} = this.props;
						...
						if (daySchedule && daySchedule.dayOfTheWeek) {
								...
								this.setState({ daysScheduled });
								setDaySchedule(daysScheduled);
						}
				}
		}

		handleDaySchedule = (dayOfTheWeek, field, value) => {
				const {setDaySchedule} = this.props;
				let daysScheduled = Object.assign([], this.state.daysScheduled);
		...
		isChecked = (dayOfTheWeek) => {
				const {daysScheduled} = this.state;
				let isChecked = false;
    ...
    handleSetAll = (schoolDays) => {
		...
		render() {
				const {companyConfig={}, classPeriods, durationOptions} = this.props;
				const {daysScheduled=[]} = this.state;
  			...
  			return (
                ...
                {daysScheduled && daysScheduled.length === 0 &&
                    <a onClick={() => this.handleSetAll(schoolDays)} className={classes(styles.row, globalStyles.link, styles.setAllSpace)}>
                        - set all -
                    </a>
                }
								{schoolDays && schoolDays.length > 0 && schoolDays.map((m,i) => {
										...
										return (
												<div key={i} className={classes(styles.rowNowrap, (i % 2 === 0 ? styles.lightGray : ''))}>
										                ...
										                id={m.label}
										                label={m.name}
										                checked={this.isChecked(m.label) || ''}
										                onClick={() => this.toggleCheckbox(m.label)}
										                labelClass={styles.labelCheckbox}
										                checkboxClass={styles.checkbox} />
														</div>
														{this.isChecked(m.label) &&
																<div className={styles.rowWrap}>
																		<div>
																				<SelectSingleDropDown
																						...
																						height={`medium-short`}
																						onChange={(event) => this.setReoccurringByClassPeriod(m.label, event.target.value)}/>
																		</div>
																		<div className={styles.dateColumn}>
																				<span className={styles.label}><L p={p} t={`Start time`}/></span>
																				<input type="time" onChange={(event) => this.handleDaySchedule(m.label, 'startTime', event.target.value)}
																		...
																		<div className={styles.duration}>
																				<SelectSingleDropDown
																						...
																						height={`medium`}
																						onChange={(event) => this.handleDaySchedule(m.label, 'duration', event.target.value)} />
																		</div>
																</div>
														}
												</div>
