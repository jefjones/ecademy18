...
import styles from './InputGradebookMultipleEntry.css';
...
export default class InputGradebookMultipleEntry extends Component {
    constructor(props) {
        super(props);
    ...
    handleChange = (event) => {
		...
		handleSetStandardLevel = (chosenOption) => {
				this.setState({ chosenOption });
		}

		validateClick = () => {
				const {isLevelOnly, onClick} = this.props;
				const {chosenOption} = this.state;
				let hasError = false;
				if (!chosenOption && Number(chosenOption) !== 0) {
						this.setState({ error: isLevelOnly ? <L p={p} t={`Please choose an option`}/> : <L p={p} t={`Please enter an option`}/>})
						hasError = true;
				}
				if (!hasError) {
    ...
    render() {
        const {handleClose, className, headerClass, explainClass, heading, explain, isLevelOnly, standardsRatings} = this.props;
        const {chosenOption} = this.state;
                    ...
                    <ModalDialog onClose={handleClose} className={styles.maxWidth}>
                        <div className={classes(styles.dialogHeader, headerClass)}>{heading}</div>
                                      ...
                                      {standardsRatings && standardsRatings.length > 0 && standardsRatings.map((m, i) =>
                                          <div key={i} onClick={() => this.handleSetStandardLevel(i+1*1)} className={classes((chosenOption === i+1*1 ? styles.border : ''), styles.space)}>
                        ...
                        <div className={styles.dialogButtons}>
		                            ...
		                            <a className={styles.noButton} onClick={handleClose}><L p={p} t={`Cancel`}/></a>
																<div>
																		<ButtonWithIcon label={<L p={p} t={`Save`}/>} icon={'checkmark_circle'} onClick={this.validateClick}/>
																</div>
														</div>
                        </div>
                    </ModalDialog>
