...
import styles from './TranscriptAddView.css';
...
export default class TranscriptAddView extends Component {
  constructor(props) {
    super(props);
  ...
  processForm = (stayOrFinish) => {
      const {addOrUpdateTranscript, personId} = this.props;
			const {transcript, selectedStudent} = this.state;
      ...
      if (!hasError) {
          ...
          this.setState({
							...
							//selectedStudent: '', Don't do this since we are probably entering more transcripts for the same student.
					})
			} else {
		...
		handleChange = (event) => {
				let transcript = Object.assign({}, this.state.transcript);
    ...
    getrecord = (record) => {
      	this.setState({transcript: Object.assign({}, record)})
    }

		handleRemoveOpen = (transcriptId) => this.setState({isShowingModal_remove: true, transcriptId })
		handleRemoveClose = () => this.setState({isShowingModal_remove: false, transcriptId: ''})
    handleRemove = () => {
				const {personId, removeTranscript} = this.props;
				const {transcriptId} = this.state;
		...
		resetAll = () => {
				...
				browserHistory.push('/transcriptAdd');
		}

		editTranscript = (transcript) => this.setState({ transcript });

	  render() {
			const {personId, myFrequentPlaces, setMyFrequentPlace, gradeLevels, schoolYears, students, transcripts={}, fetchingRecord} = this.props;
			const {transcript={}, errors, isShowingModal_remove, messageInfoIncomplete, isShowingModal_missingInfo, selectedStudent} = this.state;
			...
			let data = transcripts && transcripts.courseTranscripts && transcripts.courseTranscripts.map(m => {
					let row = [
							{value: <div onClick={() => this.editTranscript(m)}><Icon pathName={'pencil0'} premium={true} className={styles.icon}/></div> },
							{value: <div onClick={() => this.handleRemoveOpen(m.transcriptId)}><Icon pathName={'trash2'} premium={true} className={styles.icon}/></div> },
	    ...
	    return (
	        <div className={styles.container}>
	                ...
	                <a className={styles.cancelLink} onClick={() => browserHistory.push('/firstNav')}><L p={p} t={`Close`}/></a>
									...
									<a className={classes(styles.moreLeft, styles.cancelLink)} onClick={this.resetAll}><L p={p} t={`Reset`}/></a>
	            </div>
							<hr/>
