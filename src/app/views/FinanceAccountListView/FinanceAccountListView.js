...
class FinanceAccountListView extends Component {
  constructor(props) {
	    super(props);
	...
	chooseRecord = (paramPersonId) => this.setState({ paramPersonId })

	resetFilters = () => this.setState({ partialNameText: '', financeGroupTableId: [], selectedStudents: [], fromAmount: '', toAmount: '' });

	handleViewOption = (value) => {
      let newState = Object.assign({}, this.state);
      newState['viewOption'] = value;
      this.setState(newState);
  }
  ...
  render() {
	    const {personId, students, myFrequentPlaces, setMyFrequentPlace, financeAccountSummaries, fetchingRecord, financeGroups, accessRoles={},
							viewOptions} = this.props;
			const {errors, isShowingModal, fileUrl, partialNameText, selectedStudents, noSiblingsFound, financeGroupTableId, paramPersonId, fromAmount,
							toAmount, isShowingModal_description, studentName, description, viewOption} = this.state;
			...
			filteredAccounts = filteredAccounts && filteredAccounts.length > 0 && filteredAccounts.map((m, i) => {
					m.personName = <div className={classes(globalStyles.cellText, (m.personId === paramPersonId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.personId)}>
														{m.label}
												 </div>;
					m.gradeLevel = <div className={classes(globalStyles.cellText, (m.personId === paramPersonId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.personId)}>
			 											{m.gradeLevelName}
			 									 </div>;
					m.billing = m.billingAccount
                          ? <div className={classes(globalStyles.cellText, (m.personId === paramPersonId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.personId)}>
      													{`$${formatNumber(m.billingAccount, true, false, 2)}`}
      											</div>
                          : <div className={classes(globalStyles.cellText, (m.personId === paramPersonId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.personId)}></div>;
