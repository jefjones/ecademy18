...
export default class BenchmarkTestStudentComparisonView extends Component {
  constructor(props) {
	    super(props);
	...
	handleChange = ({target}) => {
			const {getBenchmarkTestStudentComparison, personId} = this.props;
			if (target.value && target.value !== '0') {
					getBenchmarkTestStudentComparison(personId, target.value);
					browserHistory.push(`/benchmarkTestStudentComparison/${target.value}`)
			}
	...
	render() {
	    const {personId, benchmarkTestStudentComparison=[], standardsRatings, benchmarkTests, fetchingRecord, standardsRatingTableId, accessRoles} = this.props;
