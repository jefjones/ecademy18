...
export default class AssessmentCorrectView extends Component {
  constructor(props) {
	    super(props);
	...
	componentWillUnmount() {
			this.props.clearAssessmentQuestion();
			this.props.clearAssessmentCorrect();
	}

	submitEssayResponse = (assessmentQuestionId, teacherResponse) => {
			const {personId, teacherEssayResponse, studentPersonId, assessmentId} = this.props;
	...
	changeCorrectionTypes  = (correctionTypes) => {
			this.setState({ correctionTypes });
	}

	changeQuestionTypes  = (questionTypes) => {
			this.setState({ questionTypes });
	}

  handleLearnerChange = (event) => {
			const {getCorrectedAssessment, assessmentQuestionsInit, personId, assessmentId, courseScheduledId, assignmentId} = this.props;
      this.setState({ studentPersonId: event.target.value, score: [] });
			...
			browserHistory.push(`/assessmentCorrect/${assignmentId}/${assessmentId}/${event.target.value}/${courseScheduledId}`);
  }

	handleRetakeTestClose = () => this.setState({isShowingModal: false})
	handleRetakeTestOpen = () => this.setState({isShowingModal: true })
	handleRetakeTest = () => {
			const {retakeTest, personId, assessmentId, assignmentId} = this.props;
			retakeTest(personId, assignmentId, assessmentId);
			this.handleRetakeTestClose();
	}

	onBlurScore = (assessmentQuestionId, event) => {
			const {updateTeacherAssessmentLearnerAnswer, personId, accessRoles, assignmentId } = this.props;
  ...
  handleScore = (assessmentQuestionId, event) => {
      this.props.setLocalScore(assessmentQuestionId, event.target.value);
  }


  render() {
    const {personId, studentPersonId, studentFullName, assessment, assessmentCorrect={}, students, questionTypeFilter, standardsRatings,
						setPenspringTransfer, correctionTypeFilter, accessRoles={}, courseScheduledId, fetchingRecord, benchmarkTestId} = this.props;
		const {correctionTypes, questionTypes, isShowingModal, isSearch} = this.state;
    ...
    if (details && details.length > 0) {
		...
		let filteredQuestions = Object.assign([], this.props.assessmentQuestions);
    ...
    return (
        <div className={styles.container}>
            <div className={globalStyles.pageTitle}>
                ...
                <TextDisplay label={benchmarkTestId ? `Benchmark assessment` : `Assessment`} className={styles.textDisplay} text={assessment && assessment.name} />
								{accessRoles.learner && assessmentCorrect.canRetakeQuiz &&
										<a onClick={this.handleRetakeTestOpen} className={styles.link}>
												<L p={p} t={`Re-take`}/>
										</a>
								}
            ...
            <hr />
						...
						{canViewAnswers &&
								<a onClick={this.toggleSearch} className={classes(styles.row, styles.search)}>
						...
						{isSearch && canViewAnswers &&
								<div className={styles.backgroundGray}>
										<RadioGroup
												...
												onClick={this.changeCorrectionTypes}/>
										<hr />
										<RadioGroup
												...
												onClick={this.changeQuestionTypes}/>
				            <hr />
								</div>
						}
                ...
                let score = m.learnerAnswer && m.learnerAnswer.score;
                                ...
                                label={''}
																...
																onChange={(event) => this.handleScore(m.assessmentQuestionId, event)}
																onBlur={(event) => this.onBlurScore(m.assessmentQuestionId, event)}/>
												</div>
								}
                  ...
                  return (
                    <div key={i}>
                        <AssessmentEssay nameKey={i} question={m} personId={personId} assessmentCorrect={details} accessRoles={accessRoles}
														isTeacher={accessRoles.facilitator} isSubmitted={true}  submitEssayResponse={this.submitEssayResponse}
            ...
            <OneFJefFooter />
						{isShowingModal &&
                <MessageModal handleClose={this.handleRetakeTestClose} heading={<L p={p} t={`Re-take?`}/>}
