...
export default class CarpoolView extends Component {
    constructor(props) {
        super(props);
		...
		componentDidMount() {
				const {personId, carpoolInit} = this.props;
				this.setState({ timerId: setInterval(() => carpoolInit(personId), 60000) });
		}
		...
		componentDidUpdate() {
				...
				if (!chosenTab && carpool && carpool.myCarpools && carpool.myCarpools.length > 0)
						this.setState({ chosenTab: 'dailySchedule' })
		}
    ...
    handleEnterKey = (event) => {
		...
		changeTab = (chosenTab) => {
				this.setState({ chosenTab });
		}

		toggleOpenAddNewCarpool = (forceClose=false) => {
				this.setState({
						chosenTab: 'myCarpools',
						expandAddNewCarpool: forceClose ? 'closed' : !this.state.expandAddNewCarpool
				});
		}
    ...
    render() {
        const {personId, carpool, companyConfig={}, addOrUpdateCarpoolRequest, daysOfWeek, removeCarpoolRequest, addOrUpdateCarpoolSearchFilter,
								...
								setDriverTimeDate, toggleCarpoolRequestFinalCanDoDay, myFrequentPlaces, setMyFrequentPlace, fetchingRecord} = this.props;
        const {chosenTab, expandAddNewCarpool} = this.state;
                ...
                <div className={styles.rowWrap}>
    								...
    								<div className={styles.row}>
    										{carpool && carpool.myCarpools && carpool.myCarpools.length > 0 &&
    											 	<div onClick={() => this.changeTab('dailySchedule')}  className={classes(styles.tabs, (chosenTab === 'dailySchedule' && styles.tabChoice))}>
    										...
    										<div onClick={() => this.changeTab('myCarpools')} className={classes(styles.tabs, (chosenTab === 'myCarpools' && styles.tabChoice))}>
    										...
    										<div onClick={() => this.changeTab('carpoolRequests')}
    								...
    								{chosenTab === 'myCarpools' &&
    										<CarpoolsMine carpool={carpool} personId={personId} companyConfig={companyConfig} removeCarpool={removeCarpool}
    												...
    												toggleOpenAddNewCarpool={this.toggleOpenAddNewCarpool} setCarpoolTimeStudentAssign={setCarpoolTimeStudentAssign}
    								...
    								{chosenTab !== 'dailySchedule' && chosenTab !== 'myCarpools' &&
    										<CarpoolRequests carpool={carpool} personId={personId} companyConfig={companyConfig} daysOfWeek={daysOfWeek} daysOfWeekAll={daysOfWeekAll}
    												...
    												changeTabChosen={this.changeTab} toggleOpenAddNewCarpool={this.toggleOpenAddNewCarpool}
