...
import { selectMe, selectAssessmentQuestions, selectAssessment, selectAssessmentCorrectSameAllStudents, selectStudents, selectAccessRoles,
 					selectFetchingRecord, selectCoursesBase } from '../store.js';
...
class Container extends Component {
    componentDidMount() {
        const {getPageLangs, langCode, personId, getSameCorrectedAssessmentAllStudents, studentPersonId, assessmentId, assessmentQuestionId, assessmentQuestionsInit,
								setMyVisitedPage, getAssessment, assignmentId} = this.props;
        ...
        this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Correct Same Assessment Question for All Students`});
    }

    render() {
			const {fetchingRecord} = this.props;
	    return fetchingRecord && !fetchingRecord.assessment  ? <AssessmentCorrectSameAllView {...this.props} /> : null;
    }
}
