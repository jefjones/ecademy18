...
class GradebookSummaryView extends Component {
    constructor(props) {
        super(props);
		...
		recallPage = (event) => {
				const {getGradebookSummary, personId} = this.props;
        const {courseScheduledId} = this.props;
				let id = event && event.target && event.target.value ? event.target.value : courseScheduledId;
				browserHistory.push('/gradebookSummary/' + id);
        getGradebookSummary(personId, id);
        this.setState({ courseScheduledId: id });
    }

    render() {
      const {personId, myFrequentPlaces, setMyFrequentPlace, courses, fetchingRecord, gradebookSummary } = this.props;
