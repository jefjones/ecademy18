...
import { selectMe, selectFetchingRecord, selectStudentAssignments, selectCompanyConfig, selectCoursesScheduled, selectVoiceRecording,
					...
					selectMyFrequentPlaces, selectStandards, selectPersonConfig, selectStudentCourseAssigns} from '../store'
...
function Container(ownProps) {
  ...
  useEffect(() => {
                    ...
                    getStandards, getStudentCoursesAssigns, accessRoles} = props
  ...
  useEffect(() => {
    				...
    				const {personId, setMyVisitedPage, courseScheduledId, studentFullName, studentPersonId, studentAssignmentsInit, course} = props
    				
    
    				if (courseScheduledId && studentFullName && (!isInit || studentPersonId !== state.studentPersonId)) {
    						studentAssignmentsInit(personId, studentPersonId, courseScheduledId)
    						props.location && props.location.pathname && setMyVisitedPage(personId, {path: props.location.pathname, description: `Assignments: ${studentFullName} ${course && course.courseName}`})
    						setIsInit(true); setStudentPersonId(studentPersonId)
    				}
    		
  }, [])

  return <StudentAssignmentsView {...props} />
}
