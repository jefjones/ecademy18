...
import { selectMe, selectFetchingRecord, selectAssessmentQuestions, selectAssessment, selectQuestionTypes, selectCompanyConfig,
					selectStudentSchedule, selectAccessRoles, selectBenchmarkTests } from '../store.js';
...
class Container extends Component {
  componentDidMount() {
      const {getPageLangs, langCode, personId, setMyVisitedPage, assessmentId, assessmentQuestionsInit, getAssessment, getBenchmarkTests, assignmentId, getStandardsRating, getStandards} = this.props;
      ...
      getAssessment(personId, assessmentId);
			...
			this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Student Assessment`});
  }

  render() {
    const {assessment} = this.props;
    return assessment && assessment.assessmentId ? <AssessmentLearnerView {...this.props} /> : null;
  }
}
