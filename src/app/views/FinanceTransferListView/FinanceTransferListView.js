...
class FinanceTransferListView extends Component {
    constructor(props) {
      super(props);
    ...
    handleReverseOpen = (toPersonId, fromPersonId) => this.setState({isShowingModal_reverse: true, toPersonId, fromPersonId })
		handleReverseClose = () => this.setState({isShowingModal_reverse: false, toPersonId: '', fromPersonId: ''})
    handleReverse = () => {
				const {fromPersonId, toPersonId} = this.state;
        this.handleReverseClose();
				browserHistory.push(`/financeTransferAdd/${fromPersonId}/${toPersonId}`)
    }
		...
		handleSelectedFinanceTransferTypes = (selectedFinanceTransferTypes) => this.setState({ selectedFinanceTransferTypes });

		resetFilters = () => this.setState({ partialNameText: '', selectedStudents: [], fromDate: '', toDate: '' });

    render() {
      const {personId, financeTransfers, companyConfig={}, accessRoles, students, myFrequentPlaces, setMyFrequentPlace, fetchingRecord } = this.props;
      const {isShowingModal_reverse, partialNameText, selectedStudents, fromDate, toDate, isShowingModal_description, studentName, description,
							financeTransferId} = this.state;
			...
			filteredTransfers = filteredTransfers && filteredTransfers.length > 0 && filteredTransfers.map((m, i) => {
					m.icons = accessRoles.admin
												? <div className={classes(styles.cellText, styles.row, (m.financeTransferId === financeTransferId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.id)}>
															<div onClick={() => this.handleReverseOpen(m.fromPersonId, m.toPersonId)} className={globalStyles.link} data-rh={'Reverse this transactdion'}>
																	<Icon pathName={'reply_arrow'} premium={true} className={styles.icon}/>
															</div>
													</div>
												: <div></div>;
					m.fromPerson = <div className={classes(styles.cellText, (m.financeTransferId === financeTransferId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeTransferId)}>
											{m.fromPersonName}
									 </div>;
				  m.fromAccount = <div className={classes(styles.cellText, (m.financeTransferId === financeTransferId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeTransferId)}>
											{m.fromAccountType}
									 </div>;
					m.toPerson = <div className={classes(styles.cellText, (m.financeTransferId === financeTransferId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeTransferId)}>
											{m.toPersonName}
									 </div>;
 				  m.toAccount = <div className={classes(styles.cellText, (m.financeTransferId === financeTransferId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeTransferId)}>
											{m.toAccountType}
									 </div>;
      ...
      return (
        <div className={styles.container}>
            <div className={styles.marginLeft}>
                ...
                <MessageModal handleClose={this.handleReverseClose} heading={<L p={p} t={`Reverse this transfer?`}/>}
