...
function StudentProfileView(props) {
  ...
  const [studentAdminNoteId, setStudentAdminNoteId] = useState('')
  const [isShowingModal_removeStudentDoc, setIsShowingModal_removeStudentDoc] = useState(true)
  const [isShowingFileUpload_course, setIsShowingFileUpload_course] = useState(true)
  const [siblingPersonId, setSiblingPersonId] = useState('')

  const {personId, accessRoles, studentProfile, login, companyConfig, schoolYears, personConfig, students, studentDocuments } = props
          ...
          <div className={styles.container}>
  						...
  						{accessRoles.admin && companyConfig.urlcode !== 'Manheim' &&
  								<div className={styles.row}>
  										...
  										<div onMouseOver={handlePrintOpen} onMouseOut={handlePrintClose} className={styles.moveUpLittle}>
  												<ReactToPrint trigger={() => <a href="#" className={classes(styles.moveDownRight, styles.link, styles.row)}><Icon pathName={'printer'} premium={true} className={classes(styles.icon, styles.moveUp)}/><L p={p} t={`Print`}/></a>} content={() => componentRef}
  														onBeforePrint={handlePrintOpen} onAfterPrint={handlePrintClose}/>
  										</div>
  										{accessRoles.admin &&
  												<a onClick={handleSend} className={classes(styles.editLink, styles.moreLeft, styles.row)} data-rh={'This teacher will receive an email with login details'}>
                      ...
                      <div className={styles.classification}>Student Information</div>
                          ...
                          {!printOpen
                              ?
                                <Checkbox
                                    ...
                                    onClick={() => toggleCheckbox('selfPaced')}
                          ...
                          <div>
                              <div className={styles.classification}><L p={p} t={`Admin Entry`}/></div>
      												<div className={classes(styles.column, styles.littleLeft)}>
      														<div className={styles.checkbox}>
      																{!printOpen
                                          ? <DateTimePicker id={`fromDate`} value={withdrawnDate || studentProfile.withdrawnDate} label={<L p={p} t={`Withdrawn date`}/>} onChange={changeWithdrawnDate}/>
      																...
      																<div key={i}>
      																		<TextDisplay leftLabel={leftLabel} text={m.note}
      																				label={<div className={styles.row}>
      																										...
      																										<a onClick={() => handleRemoveNoteOpen(m.studentAdminNoteId)} className={styles.remove}>remove</a>
      																								</div>}/>
      																</div>
      														)}
  															...
  															{studentDocuments && studentDocuments.length > 0 && studentDocuments.map((f, i) => (
  																	<div key={i} className={styles.row}>
  																			<div className={styles.linkDisplay}>
  																					{accessRoles.admin &&
  																							<a onClick={() => handleRemoveStudentDocOpen(f.id)} className={styles.remove}>
  																									<L p={p} t={`remove`}/>
  																							</a>
  																					}
                            ...
                            {!printOpen &&
  													...
  													<TextDisplay leftLabel={leftLabel} label={<L p={p} t={`Email address`}/>} text={contactLink(studentProfile.primaryGuardian.personId, studentProfile.primaryGuardian.emailAddress || studentProfile.primaryGuardian.username)} hideIfEmpty={true} />
  										...
  										{studentProfile && studentProfile.secondaryGuardians && studentProfile.secondaryGuardians.length > 0 && studentProfile.secondaryGuardians.map((m, i) =>
  												<div key={i} className={styles.column}>
  													...
  													<TextDisplay leftLabel={leftLabel} label={<L p={p} t={`Email address`}/>} text={contactLink(m.personId, m.emailAddress)} hideIfEmpty={true} />
                              ...
                              {studentProfile.VaccinationBringToOffice && <div className={styles.indentedText}><L p={p} t={`Will bring to the office`}/></div>}
  								                		...
  								                		deleteFunction={handleRemoveFileUploadOpen} deleteId={studentProfile.vaccinationFileUploadId}/>
  														}
  											 </div>
  									 }
              ...
              {!printOpen &&
  						...
  						{isShowingModal_removeNote &&
  								<MessageModal handleClose={handleRemoveNoteClose} heading={<L p={p} t={`Remove Note?`}/>} isConfirmType={true}
  									 explainJSX={<L p={p} t={`Are you sure you want to remove this note?`}/>} onClick={handleRemoveNote} />
  						}
  						{isShowingModal_removeStudentDoc &&
                  <MessageModal handleClose={handleRemoveStudentDoc} heading={<L p={p} t={`Remove this course document?`}/>}
                     explainJSX={<L p={p} t={`Are you sure you want to delete this course document?`}/>} isConfirmType={true}
                     onClick={handleRemoveStudentDoc} />
              }
  						{isShowingFileUpload_course &&
                  <FileUploadModal handleClose={handleFileUploadClose}
  										title={<L p={p} t={`Student Document`}/>} label={<L p={p} t={`File for`}/>} showTitleEntry={true}
                      personId={personId} submitFileUpload={handleSubmitFile}
