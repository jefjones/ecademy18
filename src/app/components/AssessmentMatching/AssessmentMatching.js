...
export default class AssessmentMatching extends Component {
  constructor(props) {
    super(props);

    let question = props.question ? props.question : {};
    question.questionText = typeof question.questionText === 'object'
      ? question.questionText
      : question.questionText && question.questionText.length > 0
        ? question.questionText.split("~^")
        : [];
    question.toMatchText = typeof question.toMatchText === 'object'
      ? question.toMatchText
      : question.toMatchText && question.toMatchText.length > 0
        ? question.toMatchText.split("~^")
        : [];
    question.correctAnswer = typeof question.correctAnswer === 'object'
    ...
    if (question.learnerAnswer)
      question.learnerAnswer.learnerAnswer = typeof question.learnerAnswer.learnerAnswer === 'object'
        ? question.learnerAnswer.learnerAnswer
        : question.learnerAnswer && question.learnerAnswer.learnerAnswer && question.learnerAnswer.learnerAnswer.length > 0
          ? question.learnerAnswer.learnerAnswer.indexOf(',') > -1
    ...
    this.state = {
      question,
      displayEntries: (question && question.questionText && question.questionText.length) || 6,
    }
  }

  handleRemoveLineOpen = (removeIndex) => this.setState({
    isShowingModal_removeLine: true,
    removeIndex
  })
  handleRemoveLineClose = () => this.setState({isShowingModal_removeLine: false})
  handleRemoveLine = () => {
    const {removeMatchingRemoveLine} = this.props;
    const {removeIndex} = this.state;
    removeMatchingRemoveLine(removeIndex);
    this.handleRemoveLineClose();
  }

  handleAddMoreEntries = (event) => {
    let newDisplay = event.target.value;
    newDisplay = newDisplay > 26 ? 26 : newDisplay;
    this.setState({displayEntries: newDisplay});
  }

  handleLearnerAnswer = (event, index) => {
    const {personId, addOrUpdateAssessmentAnswer, assignmentId} = this.props;
    let question = Object.assign([], this.state.question)
    let learnerAnswer = (question && question.learnerAnswer && question.learnerAnswer.learnerAnswer) || [];
    learnerAnswer[index] = event.target.value;
    if (!question.learnerAnswer) question.learnerAnswer = {};
    question.learnerAnswer.learnerAnswer = learnerAnswer;
    this.setState({question});
    learnerAnswer = learnerAnswer.join(',');
    addOrUpdateAssessmentAnswer(personId, question.assessmentQuestionId, learnerAnswer, assignmentId);
  }
  ...
  render() {
    ...
    const {displayEntries, isShowingModal_removeLine} = this.state;
            ...
            <div className={styles.row}>
              <div>
                ...
                {(accessRoles.facilitator || accessRoles.admin) &&
                  ...
                  <div onClick={() => this.handleRemoveLineOpen(index)}
              ...
              <div className={styles.row}>
                <div>
                  <SelectSingleDropDown
                    ...
                    onChange={(event) => this.handleLearnerAnswer(event, index)}/>
                </div>
    ...
    return (
      <div className={styles.container}>
        ...
        {viewMode === 'AddOrUpdate' &&
          <div className={styles.moreLeft}>
            <SelectSingleDropDown
              ...
              labelClass={bigTextDisplay ? globalStyles.bigText : ''}
              selectClass={bigTextDisplay ? globalStyles.bigText : ''}
              onChange={this.handleAddMoreEntries}/>
          </div>
        }
        ...
        {isShowingModal_removeLine &&
          <MessageModal handleClose={this.handleRemoveLineClose}
