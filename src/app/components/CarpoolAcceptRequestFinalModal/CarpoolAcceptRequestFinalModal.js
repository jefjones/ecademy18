...
export default class CarpoolAcceptRequestFinalModal extends Component {
    constructor(props) {
        super(props);
    ...
    handleChange = (event) => {
		...
		processForm = (stayOrFinish) => {
	      const {acceptRequestFinal} = this.props;
        acceptRequestFinal();
	  }
		...
		handleDaysICanDrive = (event, driveDay, carpoolTimeId) => {
				const {personId, toggleCarpoolRequestFinalCanDoDay} = this.props;
				toggleCarpoolRequestFinalCanDoDay(personId, carpoolTimeId, driveDay);
		}

    render() {
        const {handleClose, carpoolRequestResponse, daysOfWeekAll} = this.props;

				let dataPickUp = []
				carpoolRequestResponse && carpoolRequestResponse.carpoolTimes && carpoolRequestResponse.carpoolTimes.length > 0
								&& carpoolRequestResponse.carpoolTimes.filter(m => m.dropOffOrPickUp === 'pickup').forEach((m, i) => {
                        ...
                        <div className={styles.dialogHeader}>{<L p={p} t={`Carpool Request Response`}/>}</div>
												...
												<div>
														<div className={styles.dropOffBackground}>
																{carpoolRequestResponse && carpoolRequestResponse.carpoolTimes && carpoolRequestResponse.carpoolTimes.length > 0
																				&& carpoolRequestResponse.carpoolTimes.filter(m => m.dropOffOrPickUp === 'dropoff').map((m, i) => {
														...
														<div className={styles.pickUpBackground}>
																{carpoolRequestResponse && carpoolRequestResponse.carpoolTimes && carpoolRequestResponse.carpoolTimes.length > 0
																				&& carpoolRequestResponse.carpoolTimes.filter(m => m.dropOffOrPickUp === 'pickup').map((m, i) => {
