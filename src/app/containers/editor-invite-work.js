...
import { selectWorkSummary, selectWorkSummaryCurrent, selectMe, selectWorkFilter, selectLanguageList, selectEditorInviteName,
            selectEditorInviteWorkAssign, selectEditorInvitePending, selectPersonConfig } from '../store.js';
...
const mergeAllProps = (store, actions) => ({
    ...store, ...actions,
    setWorkFilter: () => actions.initSourceChosen(store.owner_personId),
    setMineChosen: () => actions.initMineChosen(store.owner_personId),
    setStatusChosen: () => actions.initStatusChosen(store.owner_personId),
    setActiveChosen: () => actions.initActiveChosen(store.owner_personId),
});
...
class Container extends Component {
  ...
  componentDidMount() {
      const {personId, getPageLangs, langCode, init, owner_personId} = this.props;
      init(owner_personId);
      getPageLangs(personId, langCode, 'EditorInviteWorkView');
  }

  render() {
      if (!this.props.workSummaries) return null;
      return <EditorInviteWorkView {...this.props} />
  }
}
