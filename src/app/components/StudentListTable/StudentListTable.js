...
class StudentListTable extends Component {
    ...
    render() {
        const {personId, students, gradeLevels, shortVersion, companyConfig={}, singleAddToClipboard,
								...
								resetUserPersonClipboard, isFetchingRecord} = this.props;
				const {sortByHeadings} = this.state;
				...
				localStudents = localStudents && localStudents.length > 0 && localStudents.map((m, i) => {
						m.icons = <div className={classes(styles.row, globalStyles.cellText)} key={i}>
												...
												{!hideIcons &&
														<a onClick={() => this.sendToStudentSchedule(m.studentPersonId)} data-rh={`Student's schedule`}>
						...
						m.student = <a onClick={() => this.sendToStudentSchedule(m.studentPersonId)} className={classes(styles.link, globalStyles.cellText)}>{m.label}</a>; //We use label here since the database determines if the company is configured for the 'student name first'
