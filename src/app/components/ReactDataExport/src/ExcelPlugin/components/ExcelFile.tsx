...
class ExcelFile extends Component {
    ...
    createSheetData(sheet) {
        ...
        const sheetData = [Children.map(columns, column => column.props.label)]
        const data = typeof (sheet.props.data) === 'function' ? sheet.props.data() : sheet.props.data

        data.forEach(row => {
            const sheetRow = []

            Children.forEach(columns, column => {
    ...
    download() {
        const wb = {
            SheetNames: Children.map(this.props.children, sheet => sheet.props.name),
            Sheets: {}
        }

        Children.forEach(this.props.children, sheet => {
