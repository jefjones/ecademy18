...
export default class MyWorksView extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      showWorkId: '',
			chosenWork: '',
			workFolderMineExpanded: props.personConfig && props.personConfig.workFolderMineExpanded,
			workFolderOthersExpanded: props.personConfig && props.personConfig.workFolderOthersExpanded,
    }
  }
	...
	toggleSearchControls = () => this.setState({ showSearchControls: !this.state.showSearchControls });

	sendToWorkAddNew = () => {
			const {mineOrOthers, workFolderId} = this.state;
			browserHistory.push(`/workAddNew/folder/${mineOrOthers}/${workFolderId || emptyGuid()}`);
	}

	sendToFolderAddUpdate = () => {
			const {mineOrOthers, workFolderId} = this.state;
			browserHistory.push(`/fileFolderAddNew/${mineOrOthers}/${workFolderId || emptyGuid()}`);
	}

	chooseRecord = (chosenWork) => this.setState({ chosenWork });

	handleToggleAllExpanded = (expandAll) => {
			const {personId, updatePersonConfig, toggleAllExpanded} = this.props;
			...
			this.setState({
					workFolderMineExpanded: expandAll,
					workFolderOthersExpanded: expandAll,
			})
	}

	handleUpdatePersonConfig = (field) => {
			const {updatePersonConfig, personId} = this.props;
			const {workFolderMineExpanded, workFolderOthersExpanded} = this.state;
			if (field === 'WorkFolderMineExpanded') {
					updatePersonConfig(personId, 'WorkFolderMineExpanded', !workFolderMineExpanded)
					this.setState({ workFolderMineExpanded: !this.state.workFolderMineExpanded})
			}
			if (field === 'WorkFolderOthersExpanded') {
					updatePersonConfig(personId, 'WorkFolderOthersExpanded', !workFolderOthersExpanded)
					this.setState({ workFolderOthersExpanded: !this.state.workFolderOthersExpanded})
			}
	}
  ...
  render() {
    const {personId, fileTreeExplorer, updateFilterByField, clearFilters, workSummaries=[], filterScratch, savedFilterIdCurrent, workFilterOptions,
						...
						resolveFetchingRecordFileTreeExplorer, setGradebookScoreByPenspring, toggleExpanded, setPenspringDistributeSubmitted} = this.props;
    const { showSearchControls, chosenWork, workFolderMineExpanded, workFolderOthersExpanded } = this.state; //searchString, searchFocusIndex, searchFoundCount
        ...
        <div className={styles.container}>
            <div className={globalStyles.pageTitle}>
                <L p={p} t={`My Documents`}/>
						...
						<hr/>
						<div onClick={this.toggleSearchControls} className={classes(styles.menuItem, styles.row)}>
            ...
            {!fetchingRecord.works && (!workSummaries || workSummaries.length === 0) ? <span className={styles.noListMessage}>{`empty list`}<br/><br/></span> : ''}
						...
						<div className={classes(styles.mainFolder, styles.row)} onClick={() => this.handleUpdatePersonConfig('WorkFolderMineExpanded')}>
								...
								<div className={classes(styles.backgroundFolder, styles.row)}>
										...
										{/*<div onClick={(event) => this.openContextMenuFileFolder(event, 'Mine')} className={classes(styles.link, styles.moreLeft)}>
						...
						<div className={styles.muchLeft}>
								<FileTreeSubContents fileTreeExplorer={fileTreeExplorer.mine} isParentExpanded={workFolderMineExpanded}
										...
										chosenWork={chosenWork} chooseRecord={this.chooseRecord} setPenspringDistributeSubmitted={setPenspringDistributeSubmitted}/>
						</div>
						<div className={classes(styles.mainFolder, styles.row, styles.moreTop)} onClick={() => this.handleUpdatePersonConfig('WorkFolderOthersExpanded')}>
								...
								<div className={classes(styles.backgroundFolder, styles.row)}>
										...
										{/*<div onClick={(event) => this.openContextMenuFileFolder(event, 'Others')} className={classes(styles.link, styles.moreLeft)}>
												<Icon pathName={'menu_lines'} premium={true} className={styles.iconMenu} />
										</div>*/}
								</div>
						</div>
