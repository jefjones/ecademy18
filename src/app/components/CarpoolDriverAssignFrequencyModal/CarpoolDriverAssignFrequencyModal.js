...
export default class CarpoolDriverAssignFrequencyModal extends Component {
    constructor(props) {
        super(props);
    ...
    handleChange = (event) => {
		...
		processForm = (stayOrFinish) => {
	      const {carpool, addCarpoolDriverAssign, personId, handleClose, driverDate, driverPersonId} = this.props;
				const {frequency, weekCount, comment} = this.state;
	      let hasError = false;

				if (!frequency) {
	          hasError = true;
	          this.setState({errorSeats: <L p={p} t={`A frequency choice is required`}/> });
	      }
				if (frequency === 'repeat' && !weekCount) {
	          hasError = true;
	          this.setState({errorSeats: <L p={p} t={`A number of weeks required`}/> });
	      }

	      if (!hasError) {
	          addCarpoolDriverAssign(personId, { carpoolId: carpool.carpoolId, frequency, weekCount, driverDate, driverPersonId, comment });
          	this.setState({
								frequency: '',
								weekCount: '',
								comment: '',
    ...
    render() {
        const {handleClose, driverDate, dropOffOrPickUp, driverPersonId, carpool} = this.props;
        const {frequency, comment, errorFrequency, weekRepeat} = this.state;
                        ...
                        <div className={styles.dialogHeader}>{<L p={p} t={`Carpool Driver Assignment Frequency`}/>}</div>
												...
												<div className={styles.moreTop}>
														<RadioGroup
																			...
																			label: <div className={styles.row}>
																								...
																								<div>
																										<SelectSingleDropDown
																												...
																												height={`short`}
																												onChange={this.handleWeekRepeatChange}/>
																									</div>
																							</div>
																		}
																]}
																...
																onClick={this.handleRepeatChoice}
