...
export default class AnnouncementModal extends Component {
		...
		handleReply = (fromPersonId) => {
				const {setStudentsSelected, handleClose, announcementId} = this.props;
				setStudentsSelected([fromPersonId], announcementId);
				browserHistory.push('/announcementEdit');
				handleClose();
		}

    render() {
        const {personId, onDelete, handleClose, setStudentsSelected, messageFullThread} = this.props;
        return (
