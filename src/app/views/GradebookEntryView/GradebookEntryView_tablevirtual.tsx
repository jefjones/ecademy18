...
function GradebookEntryView(props) {
  ...
  const gradeOverwrites = (gradeOverwrites && gradeOverwrites.length > 0 && gradeOverwrites.filter(m => m.studentPersonId !== studentPersonId)) => {
    return || []
  }
  ...
  const responseData = (responseData && responseData.scoreResponses && responseData.scoreResponses.length > 0 && responseData.scoreResponses.filter(m => m.assignmentId === assignmentId)[0]) => {
    return || {}
  }
  ...
  const {personId, companyConfig={}, courses, fetchingRecord, accessRoles={}, courseEntryId, contentTypes, removeStudentResponse, schoolYears,
                ...
                courseScheduledId, setEditMode} = props
        ...
        let data = []
            ...
            //
  					...
  					// 		}
  					// }
            row.student = <div className={classes(globalStyles.cellText, styles.row, (m.withdrawnDate ? styles.tanBack : m.selfPaced ? styles.responseBack : m.id === chosenStudentRow ? styles.highlightBack : i === 0 ? styles.whiteBack : ''))} onClick={() => chooseRecord(m.courseEntryId)}
                                data-rh={m.withdrawnDate ? `Withdrawn date: ${m.withdrawnDate.substring(0, m.withdrawnDate.indexOf('T'))}` : m.selfPaced ? 'Self-paced' : null}>
      											 <div className={styles.row}>
      													 ...
      													 <div onClick={() => sendToStudentAssignmentView(m.personId)} className={classes(styles.link, styles.row)}>
            ...
            gradebookLocal && gradebookLocal.assignments && gradebookLocal.assignments.length > 0 && gradebookLocal.assignments.forEach((s, i) => {
                ...
                if (foundJumpTo && matchesFilter) {
                    ...
                    row[s.assignmentId] = <div key={i} className={classes(styles.row, (hasStudentAssignmentAssign ? '' : styles.noAssignBackground), (m.withdrawnDate ? styles.tanBack : m.selfPaced ? styles.responseBack : m.id === chosenStudentRow ? styles.highlightBack : ''))}
                                                data-rh={hasStudentAssignmentAssign ? null : 'Not assigned to this assignment'} tabIndex={-1}  onClick={() => chooseRecord(m.courseEntryId)}>
            ...
            if (!isLevelOnly && (course.gradingType === 'TRADITIONAL' || course.gradingType === 'STANDARDSRATING')) {
                row.overwrite = <div className={classes(globalStyles.cellText, styles.row, (m.withdrawnDate ? 'tan' : m.selfPaced ? 'response' : m.id === chosenStudentRow ? 'highlight' : ''))} onClick={() => chooseRecord(m.id)}>
              ...
              <div className={classes(globalStyles.pageTitle, styles.moreMarginBottom)}>
                	{`Gradebook ${course.gradingType === 'STANDARDSRATING' ? '(Standards-based)' : course.gradingType === 'PASSFAIL' ? '(Pass / Fail)' : ''}`}
  						...
  						<div className={styles.row}>
  								...
  								<ReactToPrint trigger={() => <a href="#" className={classes(styles.moreRight, styles.link)}>Print</a>} content={() => componentRef}/>
                          ...
                          <Loading isLoading={fetchingRecord.baseCourses} />
  								...
  								{gradebook && gradebook.assignments && gradebook.assignments.length > 0 && courseScheduledId && !(fetchingRecord && fetchingRecord.gradebook) &&
  										<div className={styles.row}>
  												...
  												<ButtonWithIcon icon={'checkmark_circle'} label={'Finalize'} onClick={handleFinalizeGradeOpen}/>
