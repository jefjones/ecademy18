...
import * as styles from './DoctorNoteListView.css'
...
function DoctorNoteListView(props) {
  ...
  const [doctorNoteId, setDoctorNoteId] = useState('')
  ...
  const [isShowingModal_doctor, setIsShowingModal_doctor] = useState(true)
  const [displayDoctor, setDisplayDoctor] = useState('')
  const [partialNameText, setPartialNameText] = useState(target.value)
  const [doctorPersonId, setDoctorPersonId] = useState('')
  const [studentPersonId, setStudentPersonId] = useState('')
  const [searchDate, setSearchDate] = useState('')

  const {doctorNotes={}, fetchingRecord, companyConfig, accessRoles, doctorList, studentList} = props
  		...
  		localDoctorNotes = localDoctorNotes && localDoctorNotes.length > 0 && localDoctorNotes.map((m, i) => {
  				m.filesAndNotes = <div className={classes(styles.row, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteId)} key={i}>
  															{m.fileUploads && m.fileUploads.length > 0 && m.fileUploads.map((f, i) =>
  																	<a onClick={() => handleDocumentOpen(m.doctorNoteId, f)} key={i}>
  																			<Icon pathName={'document0'} premium={true} className={styles.iconCell} />
  																	</a>)
  															}
  															{m.note &&
  																	<div onClick={() => handleInstructionsOpen(m.doctorNoteId, m.note)}>
  																			<Icon pathName={'comment_edit'} premium={true} className={styles.iconCell} />
  																	</div>
  															}
  												 </div>
  				m.doctorOffice = <div className={classes(styles.cellText, globalStyles.link, m.isChosenRecord ? styles.highlight : '')} onClick={() => {handleEnteredByOpen(m); chooseRecord(m.doctorNoteId);}}>
  														{m.doctorOfficeName}
  												 </div>
  			 	m.student = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteId)}>
  												{m.studentName}
  										</div>
  			 	m.from = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteId)}>
  										<DateMoment date={m.fromDate} includeTime={false}/>
  								</div>
  			 	m.to = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteId)}>
  										<DateMoment date={m.toDate} includeTime={false}/>
  								</div>
  				m.doctor = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteId)}>
  										{m.doctorName}
  								</div>
  			 	m.entryPerson = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteId)}>
  										{m.dataEntryName}
  								</div>
  			 	m.dateOfEntry = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteId)}>
              ...
              <OneFJefFooter />
  						{isShowingModal_instructions &&
  								<MessageModal handleClose={handleInstructionsClose} heading={<L p={p} t={`Excused Absence Note`}/>}
  									 onClick={handleInstructionsClose} explain={note}  />
  						}
  						...
  						{isShowingModal_instructions &&
  								<MessageModal handleClose={handleInstructionsClose} heading={<L p={p} t={`Doctor Office`}/>}
  									 onClick={handleInstructionsClose} explain={doctorNoteText}  />
  						}
        </div>
      )
}
