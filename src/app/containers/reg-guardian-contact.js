...
import { selectMe, selectRegistration, selectMaritalStati, selectGender, selectCountries, selectUSStates } from '../store.js';
...
class Container extends Component {
	componentDidMount() {
			const {getPageLangs, langCode, personId, setMyVisitedPage, getCountries, getUSStates, getGender, getMaritalStatus} = this.props;
			getPageLangs(personId, langCode, 'RegGuardiansContactsView');
			getCountries();
			getUSStates();
			getMaritalStatus(personId);
			getGender();
      this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Guardian Contacts`});
	}

	  render() {
	    	return <RegGuardiansContactsView {...this.props} />;
	  }
}
