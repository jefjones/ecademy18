...
import * as actionCourseFilter from '../actions/course-filter'
...
import { selectLearningPathways, selectUsers, selectStudents, selectMe, selectLearnerFilter, selectCoursesScheduled,
					...
					selectCourseClipboard, selectMyFrequentPlaces, selectFetchingRecord } from '../store'
...
function Container(ownProps) {
  ...
  useEffect(() => {
            ...
            const {getPageLangs, langCode, personId, setMyVisitedPage, getUserPersonClipboard, getCourseClipboard, companyConfig, accessRoles} = props
            getPageLangs(personId, langCode, 'LearnerCourseAssignView')
    				...
    				getCourseClipboard(personId, companyConfig.features && companyConfig.features.selfServiceStudentSignup && (accessRoles.facilitator || accessRoles.counselor)
    						? 'courseEntry'
    						: 'courseScheduled')
    				props.location && props.location.pathname && props.location && props.location.pathname && setMyVisitedPage(personId, {path: props.location.pathname, description: `Assign Course to Student`})
        
  }, [])

  return <LearnerCourseAssignView {...props} />
}
