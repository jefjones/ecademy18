...
import { selectMe, selectFinanceWaiverSchedules, selectFetchingRecord } from '../store.js';
...
class Container extends Component {
    componentDidMount() {
        const {getPageLangs, langCode, setMyVisitedPage, getFinanceWaiverSchedules, personId} = this.props;
        getPageLangs(personId, langCode, 'FinanceWaiverSchedulesView');
        getFinanceWaiverSchedules(personId);
        this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Waiver Schedule (Finance)`});
    }

    render() {
        return <FinanceWaiverSchedulesView {...this.props} />;
    }
}
