...
class FinanceBillingListView extends Component {
    constructor(props) {
      super(props);

      this.state = {
					financeBillingId: '',
					onlyBillingDue: props.onlyBillingDue === 'onlyBillingDue',
      }
    }
		...
		handleDescriptionOpen = (billedPersonName, description) => this.setState({isShowingModal_description: true, billedPersonName, description })
    handleDescriptionOpenClose = () => this.setState({isShowingModal_description: false, billedPersonName: '', description: '' })

    handleRemoveOpen = (financeBillingId) => this.setState({isShowingModal_remove: true, financeBillingId })
		handleRemoveClose = () => this.setState({isShowingModal_remove: false, financeBillingId: ''})
    handleRemove = () => {
				const {personId, removeFinanceBilling} = this.props;
				const {financeBillingId} = this.state;
        removeFinanceBilling(personId, financeBillingId);
        this.handleRemoveClose();
        this.props.alert.info(<div className={globalStyles.alertText}><L p={p} t={`The billing record has been removed.`}/></div>)
    }
		...
		toggleCheckbox = () => this.setState({ onlyBillingDue: !this.state.onlyBillingDue });

		resetFilters = () => this.setState({ partialNameText: '', selectedStudents: [], selectedFinanceFeeTypes: [], fromDate: '', toDate: '' });
    ...
    render() {
      ...
      const {isShowingModal_remove, partialNameText, selectedStudents, selectedFinanceFeeTypes, fromDate, toDate, isShowingModal_description,
							billedPersonName, description, financeBillingId, onlyBillingDue} = this.state;
			...
			filteredBillings = filteredBillings && filteredBillings.length > 0 && filteredBillings.map((m, i) => {
					m.icons = !m.isPaid
												? accessRoles.admin
                              ? <div className={classes(styles.cellText, styles.littleTop, styles.row)} onClick={() => this.chooseRecord(m.financeBillingId)}>
					...
					m.fee = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
											{m.financeFeeTypeName}
									 </div>;
					m.name = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
											{m.billedPersonName}
									 </div>;
					m.billingAmount = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
				 ...
				 m.glcodeName = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
														{m.financeGlcodeName}
												</div>;
				 m.refund = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
												{m.refundType === 'NotRefundable' ? 'Not Refundable' : m.refundType === '100Refundable' ? '100% refundable' : 'Refund schedule'}
										</div>;
				 m.lowIncomeWaiverName = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
																{m.financeLowIncomeWaiverName}
														 </div>;
				 m.mandatory = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
																{m.mandatoryOrOptional}
														 </div>;
				 m.due = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
																<DateMoment date={m.dueDate}/>
														 </div>;
					m.group = <div onClick={() => this.chooseRecord(m.financeBillingId)} className={classes(styles.cellText, styles.link, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))}>
												{m.financeGroupTableName}
										</div>;
					m.desc = <div className={classes(styles.cellText, globalStyles.link, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => {this.handleDescriptionOpen(m.billedPersonName, m.description); this.chooseRecord(m.financeBillingId);}}>
												{m.description && m.description.length > 50 ? m.description.substring(0,50) + '...' : m.description}
										</div>;
          m.partial = m.partialPayment
                          ? <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
																{`$${formatNumber(m.partialPayment, true, false, 2)}`}
														</div>
                          : '';
         m.partialDesc = <div className={classes(styles.cellText, globalStyles.link, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => {this.handleDescriptionOpen(m.billedPersonName, m.partialPaymentComment); this.chooseRecord(m.financeBillingId);}}>
												{m.partialPaymentComment && m.partialPaymentComment.length > 50 ? m.partialPaymentComment.substring(0,50) + '...' : m.partialPaymentComment}
										</div>;
					m.entry = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
												<DateMoment date={m.entryDate}/>
										</div>;
					m.entryPerson = <div className={classes(styles.cellText, (m.financeBillingId === financeBillingId ? globalStyles.highlight : ''))} onClick={() => this.chooseRecord(m.financeBillingId)}>
															{m.entryPersonName}
													</div>;
      ...
      return (
        <div className={styles.container}>
            <div className={styles.marginLeft}>
                ...
                <MessageModal handleClose={this.handleDescriptionOpenClose} heading={billedPersonName} explain={description} onClick={this.handleDescriptionOpenClose} />
            }
        </div>
    )};
}
