...
function AssessmentCorrectSummaryView(props) {
  ...
  const {assignmentId, assessmentId, correctSummary, correctDetails={}, assessmentQuestions, students} = props
  			...
  			students && students.length > 0 && students.forEach(m => {
  					let row = [{value: <div className={styles.rowSpace}>
  										 <div className={styles.row}>
  												 ...
  												 <div onClick={() => sendToStudentSchedule(m.id)} className={classes(styles.link, styles.row)}>
  														{m.label}
  												</div>
        ...
        return (
          <div className={styles.container}>
              ...
              <div className={styles.tableMargin} ref={el => (componentRef = el)}>
  								<EditTableFreezeLeft labelClass={styles.tableLabelClass} headings={headings} data={data}
  										firstColumnClass={styles.firstColumnClass} sendToReport={handlePathLink}/>
              </div>
              <OneFJefFooter />
          </div>
      )
