...
import * as styles from './FinanceCreditTypesView.css'
...
function FinanceCreditTypesView(props) {
  ...
  const {financeCreditTypes, fetchingRecord} = props
      ...
      if (financeCreditTypes && financeCreditTypes.length > 0) {
          data = financeCreditTypes.map(m => {
              return ([
  							{value: m.name && <a onClick={() => handleEdit(m.financeCreditTypeId)}><Icon pathName={'pencil0'} premium={true} className={styles.icon}/></a>},
                {value: m.name && <a onClick={() => handleRemoveItemOpen(m.financeCreditTypeId, m.usedCount)}><Icon pathName={'trash2'} premium={true} className={styles.icon}/></a>},
  							{value: m.name},
  							{value: m.description},
                {value: m.usedCount},
              ])
      ...
      return (
          <div className={styles.container}>
              ...
              {isShowingModal_remove &&
                  <MessageModal handleClose={handleRemoveItemClose} heading={<L p={p} t={`Remove this finance credit type?`}/>}
                  ...
                  <MessageModal handleClose={handleShowUsedCountClose} heading={<L p={p} t={`This Finance Credit Type is in Use`}/>}
...
export default FinanceCreditTypesView
