...
import { selectMe, selectStudents, selectUsers, selectCompanyConfig, selectGradeLevels, selectTheStudent } from '../store'
...
function Container(ownProps) {
  ...
  useEffect(() => {
            ...
            const {getPageLangs, langCode, setMyVisitedPage, personId, getGradeLevels, getTheStudent, studentPersonId} = props
            getPageLangs(personId, langCode, 'LearnerAddView')
    				getGradeLevels()
            getTheStudent(personId, studentPersonId)
            props.location && props.location.pathname && setMyVisitedPage(personId, {path: props.location.pathname, description: `Add Student`})
    		
  }, [])

  return <LearnerAddView {...props} />
}
