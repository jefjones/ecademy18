...
import { selectMe, selectMyFrequentPlaces, selectRegistrationPending, selectPersonConfig, selectFetchingRecord, selectGradeLevels,
					selectCompanyConfig } from '../store.js';
...
class Container extends Component {
		componentDidMount() {
				const {getPageLangs, langCode, setMyVisitedPage, getRegistrationPending, personId} = this.props;
				getPageLangs(personId, langCode, 'RegistrationPendingView');
				getRegistrationPending(personId);
				this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Pending Registration`});
		}

	  render() {
	    	return <RegistrationPendingView {...this.props} />;
	  }
}
