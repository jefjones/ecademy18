...
export default class extends Component {
	  constructor(props) {
		...
		// componentDidUpdate() {
		// 	const {fileTreeExplorer} = this.props;
		// 		const {isScoreInit} = this.state;
		// 		let scores = [];
		// 		if (!isScoreInit && fileTreeExplorer && fileTreeExplorer.files && fileTreeExplorer.files.length > 0) {
		// 				fileTreeExplorer.files.forEach(m => {if (!!m.studentAssignmentResponseId) scores[m.studentAssignmentResponseId] = m.score });
		// 				this.setState({ scores, isScoreInit: true });
		// 		}
		// }

		sendToReview = (workId, chapterId, languageId) => {
				const {personId, setWorkCurrentSelected, getAuthorWorkspace} = this.props;
		...
		handlePenspringHomeworkOpen = (workId) => this.setState({ workId, isShowingPenspringHomework: true})
    handlePenspringHomeworkClose = () => this.setState({workId: '', isShowingPenspringHomework: false})
		handlePenspringHomework = () => {
				const {personId, setPenspringHomeworkSubmitted, getMyWorks, getWorksSharedWithMe} = this.props;
		...
		handlePenspringDistributeOpen = (workId) => this.setState({ workId, isShowingPenspringDistribute: true})
    handlePenspringDistributeClose = () => this.setState({workId: '', isShowingPenspringDistribute: false})
		handlePenspringDistribute = () => {
				const {personId, setPenspringDistributeSubmitted} = this.props;
				const {workId} = this.state;
				setPenspringDistributeSubmitted(personId, workId);
				this.handlePenspringDistributeClose();
		}

		onBlurScore = (studentAssignmentResponseId, event) => {
				const {setGradebookScoreByPenspring, personId } = this.props;
				setGradebookScoreByPenspring(personId, studentAssignmentResponseId, event.target.value);
		}

		handleScore = (studentAssignmentResponseId, event) => {
		...
		render() {
				const {fileTreeExplorer, isParentExpanded=true, toggleExpanded, personId, setWorkCurrentSelected, workSummaries, deleteWork, mineOrOthers,
								...
								setGradebookScoreByPenspring, chooseRecord, chosenWork, setPenspringDistributeSubmitted } = this.props;
				const {isShowingPenspringHomework, scores, isShowingPenspringDistribute} = this.state;
		    ...
		    return !isParentExpanded ? null : (
		        <div className={styles.indent}>
								...
								<table className={styles.document}>
									<tbody>
											{fileTreeExplorer && fileTreeExplorer.files && fileTreeExplorer.files.length > 0 && fileTreeExplorer.files.map((m, i) => {
												...
												return (
													<tr key={i}>
														<td onClick={() => this.sendToReview(m.workId, m.chapterId, m.languageId)} className={styles.cellStyle}>
																				...
																				? m.isDistributableAssignment
																						...
																						: m.penspringSubmittedDate
																								...
																								: m.isPenspringHomework || (m.studentAssignmentResponseId && m.studentAssignmentResponseId !== guidEmpty)
																										? <a className={styles.linkBold} onClick={() => this.handlePenspringHomeworkOpen(m.workId)}><L p={p} t={`Submit Homework`}/></a>
																										: ''
																				: m.penspringSubmittedDate
																						? <DateMoment date={m.penspringSubmittedDate} format={'D MMM  h:mm a'} className={styles.date}/>
																						: ''
																		...
																		: m.isPenspringSubmitted && (m.canEditScore || m.accessRoleName === 'Facilitator' || m.gradingType === 'STUDENT')
																				? <div className={styles.inputBox}>
																						<InputText id={i} tabIndex={i} size={"super-short"}
																								...
																								numberOnly={true}
																								onChange={(event) => this.handleScore(m.studentAssignmentResponseId, event)}
																								onBlur={(event) => this.onBlurScore(m.studentAssignmentResponseId, event)} />
																					</div>
		                ...
		                <MessageModal handleClose={this.handlePenspringDistributeOpen} heading={<L p={p} t={`Distribute this homework?`}/>}
