...
import RegBillingPreferenceViewSimple from '../views/RegBillingPreferenceView_simple';
...
import { selectMe, selectRegistration, selectUSStates, selectCountries, selectPaymentProcessingResponse, selectCompanyConfig, selectProxynizationTempPassword } from '../store.js';
...
const mapStateToProps = (state, props) => {
    ...
    return {
        ...
        proxynizationTempPassword: selectProxynizationTempPassword(state),
    }
};
...
const bindActionsToDispatch = dispatch => ({
    ...
    getProxynizationTempPassword: () => dispatch(actionRegBillingPreference.getProxynizationTempPassword()),
});
...
class Container extends Component {
		componentDidMount() {
				const {getPageLangs, langCode, personId, setMyVisitedPage, getCountries, getUSStates, getProxynizationTempPassword} = this.props;
				getPageLangs(personId, langCode, 'RegBillingPreferenceView_simple');
				getCountries();
				getUSStates();
        this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Billing Preference`});
        getProxynizationTempPassword();
		}

	  render() {
	    	return <RegBillingPreferenceViewSimple {...this.props} />;
	  }
}
