...
import * as styles from './DoctorNoteInviteListView.css'
...
function DoctorNoteInviteListView(props) {
  ...
  const {doctorNoteInvites={}, fetchingRecord, accessRoles, doctorList, studentList, doctorEmailAddressList} = props
  		...
  		localDoctorNoteInvites = localDoctorNoteInvites && localDoctorNoteInvites.length > 0 && localDoctorNoteInvites.map((m, i) => {
  				m.icons = <div className={classes(styles.row, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteInviteId)} key={i}>
  											{accessRoles.doctor &&
  													<div className={styles.buttonCell}>
  															<ButtonWithIcon icon={'checkmark_circle'} label={''} onClick={() => navigate(`/doctorNoteAdd/${m.doctorNoteInviteId}`)} addClassName={styles.smallButton}/>
  													</div>
  											}
  											<a key={i} onClick={() => handleDeleteOpen(m.doctorNoteInviteId)}>
  													<Icon pathName={'trash2'} premium={true} className={styles.iconCell} />
  											</a>
  								  </div>
  				m.doctorOffice = <div className={classes(styles.cellText, globalStyles.link, m.isChosenRecord ? styles.highlight : '')} onClick={() => {handleEnteredByOpen(m); chooseRecord(m.doctorNoteInviteId);}}>
  														{m.doctorOfficeName}
  												 </div>
  			  m.doctorEmail = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')}>
  														{m.doctorEmailAddress}
  												 </div>
  			 	m.student = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteInviteId)}>
  												{m.studentName}
  										</div>
  			 	m.from = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteInviteId)}>
  										<DateMoment date={m.fromDate} includeTime={false}/>
  								</div>
  			 	m.to = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteInviteId)}>
  										<DateMoment date={m.toDate} includeTime={false}/>
  								</div>
  			 	m.dateOfEntry = <div className={classes(styles.cellText, m.isChosenRecord ? styles.highlight : '')} onClick={() => chooseRecord(m.doctorNoteInviteId)}>
          ...
          <div className={styles.container}>
              ...
              <OneFJefFooter />
  						{isShowingModal_delete &&
  								<MessageModal handleClose={handleDeleteClose} heading={<L p={p} t={`Delete Doctor Note Invite`}/>}
  									 onClick={handleDelete} explainJSX={<L p={p} t={`Are you sure you want to delete this doctor note invite?`}/>} isConfirmType={true}  />
  						}
  						{isShowingModal_doctor &&
  								<MessageModal handleClose={handleEnteredByClose} heading={`Doctor's Office`} onClick={handleEnteredByClose}
  										explainJSX={<div>
