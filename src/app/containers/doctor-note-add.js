...
import { selectMe, selectSchools, selectStudents, selectDoctorNoteInvites } from '../store.js';
...
class Container extends Component {
    componentDidMount() {
        const {getPageLangs, langCode, personId, setMyVisitedPage, getSchools} = this.props;
        getPageLangs(personId, langCode, 'DoctorNoteAddView');
        getSchools(personId);
        this.props.location && this.props.location.pathname && setMyVisitedPage(personId, {path: this.props.location.pathname, description: `Add Doctor Note`});
    }

    render() {
        return <DoctorNoteAddView {...this.props} />;
    }
}
