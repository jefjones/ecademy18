...
export default class AssessmentCorrectSameAllView extends Component {
  constructor(props) {
	    super(props);
	...
	componentDidUpdate() {
			const {assessmentCorrectSameAllStudents} = this.props;
			const {isInitScore} = this.state;
			...
			if (!isInitScore && assessmentCorrectSameAllStudents && assessmentCorrectSameAllStudents.length > 0) {
					...
					this.setState({ isInitScore: true, scores });
			}
	}
  ...
  handleQuestionChange = (event) => {
      const {getSameCorrectedAssessmentAllStudents, personId, assessmentId, assignmentId} = this.props;
      this.setState({ assessmentQuestionId: event.target.value, isInitScore: false, scores: [] });
      getSameCorrectedAssessmentAllStudents(personId, assessmentId, event.target.value);
			browserHistory.push(`/assessmentCorrectSameAll/${assignmentId}/${assessmentId}/${event.target.value}`)
  }

	onBlurScore = (studentPersonId, event) => {
			const {updateTeacherAssessmentLearnerAnswer, personId, accessRoles, assessmentQuestionId, assignmentId } = this.props;
  ...
  render() {
    const {personId, assessment, assessmentCorrectSameAllStudents, assessmentQuestions, setPenspringTransfer, correctionTypeFilter,
						accessRoles={}, course, assessmentId, assignmentId} = this.props;
		const {correctionTypes, isShowingModal, isSearch, scores=[], assessmentQuestionId} = this.state;
        ...
        <div className={styles.container}>
            <div className={globalStyles.pageTitle}>
                ...
                <TextDisplay label={<L p={p} t={`Assessment`}/>} className={styles.textDisplay} text={assessment && assessment.name} />
								{accessRoles.learner &&
										<a onClick={this.handleRetakeTestOpen} className={classes(styles.link, styles.row)}>
												...
												correctControls =
														<div>
																...
																<InputText size={"super-short"}
																		...
																		onChange={(event) => this.handleScore(studentAnswer.personId, event)}
																		onBlur={(event) => this.onBlurScore(studentAnswer.personId, event)}/>
														</div>
										}
		                  ...
		                  return (
		                    <div key={i}>
		                        ...
		                        <AssessmentEssay nameKey={i} question={m} personId={personId} assessmentCorrect={assessmentCorrectSameAllStudents}
																accessRoles={accessRoles} isTeacher={accessRoles.facilitator} isSubmitted={true} submitEssayResponse={this.submitEssayResponse}
            ...
            <OneFJefFooter />
						{isShowingModal &&
                <MessageModal handleClose={this.handleRetakeTestClose} heading={<div className={styles.row}><L p={p} t={`Re-take `}/>{assessment && assessment.courseContentTypeName}</div>}
