...
class FinancePaymentReceiptView extends Component {
    constructor(props) {
      super(props);
    ...
    handleDescriptionOpenClose = () => this.setState({isShowingModal_description: false, studentName: '', description: '' })
		...
		handleChange = ({target}) => {
				const {personId, getFinancePaymentBillings} = this.props;
				this.setState({ financePaymentTableId: target.value });
				getFinancePaymentBillings(personId, target.value);
				browserHistory.push(`/financePaymentReceipt/${target.value}`)
		}

    render() {
      const {personId, financeBillings=[], financePayments=[], financePayment={}, companyConfig={}, myFrequentPlaces, setMyFrequentPlace, fetchingRecord } = this.props;
			const {financePaymentTableId} = this.state;
